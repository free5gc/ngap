...
type PRSConfigurationRequest struct {
	NRPPATransactionID   int64                    // mandatory
	PRSConfigRequestType *ie.PRSConfigRequestType // mandatory
	PRSTRPList           *ie.PRSTRPList           // mandatory,
}

func (x *PRSConfigurationRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PRSConfigurationRequest) ProcedureCode() int64 {
	return ProcedureCodePRSConfigurationExchange
}

func (x *PRSConfigurationRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PRSConfigurationRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: PRSConfigurationRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PRSConfigRequestType != nil {
		err = x.PRSConfigRequestType.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPRSConfigRequestType},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PRSConfigRequestType is missing")
	}

	// IE Field 2 (mandatory,)
	if x.PRSTRPList != nil {
		err = x.PRSTRPList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPRSTRPList},
...
func (x *PRSConfigurationRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PRSConfigurationRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDPRSConfigRequestType {
			// check if ie is duplicated
			if x.PRSConfigRequestType != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PRSConfigRequestType")
			...
			x.PRSConfigRequestType = &ie.PRSConfigRequestType{}
			err = x.PRSConfigRequestType.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDPRSTRPList {
			// check if ie is duplicated
			if x.PRSTRPList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PRSTRPList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PRSTRPList = &ie.PRSTRPList{}
			err = x.PRSTRPList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PRSConfigRequestType == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPRSConfigRequestType, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PRSConfigRequestType is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
