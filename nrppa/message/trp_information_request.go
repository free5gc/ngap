...
type TRPInformationRequest struct {
	...
	TRPList                      *ie.TRPList                      // optional
	TRPInformationTypeListTRPReq *ie.TRPInformationTypeListTRPReq // mandatory,
}

func (x *TRPInformationRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *TRPInformationRequest) ProcedureCode() int64 {
	return ProcedureCodeTRPInformationExchange
}

func (x *TRPInformationRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *TRPInformationRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: TRPInformationRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 1 (optional)
	if x.TRPList != nil {
		err = x.TRPList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPList},
	...
	if x.TRPInformationTypeListTRPReq != nil {
		err = x.TRPInformationTypeListTRPReq.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPInformationTypeListTRPReq},
...
func (x *TRPInformationRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (TRPInformationRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDTRPList {
			// check if ie is duplicated
			if x.TRPList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPList")
			...
			x.TRPList = &ie.TRPList{}
			err = x.TRPList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDTRPInformationTypeListTRPReq {
			// check if ie is duplicated
			if x.TRPInformationTypeListTRPReq != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPInformationTypeListTRPReq")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TRPInformationTypeListTRPReq = &ie.TRPInformationTypeListTRPReq{}
			err = x.TRPInformationTypeListTRPReq.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
