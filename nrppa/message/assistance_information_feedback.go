...
type AssistanceInformationFeedback struct {
	...
	AssistanceInformationFailureList *ie.AssistanceInformationFailureList // optional
	PositioningBroadcastCells        *ie.PositioningBroadcastCells        // optional
	CriticalityDiagnostics           *ie.CriticalityDiagnostics           // optional,
}

func (x *AssistanceInformationFeedback) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *AssistanceInformationFeedback) ProcedureCode() int64 {
	return ProcedureCodeAssistanceInformationFeedback
}

func (x *AssistanceInformationFeedback) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *AssistanceInformationFeedback) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: AssistanceInformationFeedback (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 1 (optional)
	if x.AssistanceInformationFailureList != nil {
		err = x.AssistanceInformationFailureList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDAssistanceInformationFailureList},
...
func (x *AssistanceInformationFeedback) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (AssistanceInformationFeedback) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDAssistanceInformationFailureList {
			// check if ie is duplicated
			if x.AssistanceInformationFailureList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: AssistanceInformationFailureList")
			...
			x.AssistanceInformationFailureList = &ie.AssistanceInformationFailureList{}
			err = x.AssistanceInformationFailureList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
