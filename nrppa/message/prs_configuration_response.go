...
type PRSConfigurationResponse struct {
	NRPPATransactionID     int64                      // mandatory
	PRSTransmissionTRPList *ie.PRSTransmissionTRPList // optional
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional,
}

func (x *PRSConfigurationResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *PRSConfigurationResponse) ProcedureCode() int64 {
	return ProcedureCodePRSConfigurationExchange
}

func (x *PRSConfigurationResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PRSConfigurationResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: PRSConfigurationResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 1 (optional)
	if x.PRSTransmissionTRPList != nil {
		err = x.PRSTransmissionTRPList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPRSTransmissionTRPList},
...
func (x *PRSConfigurationResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PRSConfigurationResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDPRSTransmissionTRPList {
			// check if ie is duplicated
			if x.PRSTransmissionTRPList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PRSTransmissionTRPList")
			...
			x.PRSTransmissionTRPList = &ie.PRSTransmissionTRPList{}
			err = x.PRSTransmissionTRPList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
