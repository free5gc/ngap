...
type TRPInformationResponse struct {
	...
	TRPInformationListTRPResp *ie.TRPInformationListTRPResp // mandatory
	CriticalityDiagnostics    *ie.CriticalityDiagnostics    // optional,
}

func (x *TRPInformationResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *TRPInformationResponse) ProcedureCode() int64 {
	return ProcedureCodeTRPInformationExchange
}

func (x *TRPInformationResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *TRPInformationResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: TRPInformationResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.TRPInformationListTRPResp != nil {
		err = x.TRPInformationListTRPResp.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPInformationListTRPResp},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field TRPInformationListTRPResp is missing")
	}
...
func (x *TRPInformationResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (TRPInformationResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDTRPInformationListTRPResp {
			// check if ie is duplicated
			if x.TRPInformationListTRPResp != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPInformationListTRPResp")
			...
			x.TRPInformationListTRPResp = &ie.TRPInformationListTRPResp{}
			err = x.TRPInformationListTRPResp.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// TRPInformationListTRPResp is mandatory but may be nil (ignored)
