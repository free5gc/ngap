...
type MeasurementReport struct {
	...
	TRPMeasurementReportList *ie.TRPMeasurementResponseList // mandatory,
}

func (x *MeasurementReport) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *MeasurementReport) ProcedureCode() int64 {
	return ProcedureCodeMeasurementReport
}

func (x *MeasurementReport) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *MeasurementReport) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementReport (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.TRPMeasurementReportList != nil {
		err = x.TRPMeasurementReportList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPMeasurementReportList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *MeasurementReport) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementReport) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDTRPMeasurementReportList {
			// check if ie is duplicated
			if x.TRPMeasurementReportList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPMeasurementReportList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TRPMeasurementReportList = &ie.TRPMeasurementResponseList{}
			err = x.TRPMeasurementReportList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
