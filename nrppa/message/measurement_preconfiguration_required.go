...
type MeasurementPreconfigurationRequired struct {
	NRPPATransactionID    int64                     // mandatory
	TRPPRSInformationList *ie.TRPPRSInformationList // mandatory,
}

func (x *MeasurementPreconfigurationRequired) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *MeasurementPreconfigurationRequired) ProcedureCode() int64 {
	return ProcedureCodeMeasurementPreconfiguration
}

func (x *MeasurementPreconfigurationRequired) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *MeasurementPreconfigurationRequired) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementPreconfigurationRequired (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.TRPPRSInformationList != nil {
		err = x.TRPPRSInformationList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPPRSInformationList},
...
func (x *MeasurementPreconfigurationRequired) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementPreconfigurationRequired) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDTRPPRSInformationList {
			// check if ie is duplicated
			if x.TRPPRSInformationList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPPRSInformationList")
			...
			x.TRPPRSInformationList = &ie.TRPPRSInformationList{}
			err = x.TRPPRSInformationList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
