...
type MeasurementPreconfigurationRefuse struct {
	NRPPATransactionID     int64                      // mandatory
	Cause                  *ie.Cause                  // mandatory
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional,
}

func (x *MeasurementPreconfigurationRefuse) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *MeasurementPreconfigurationRefuse) ProcedureCode() int64 {
	return ProcedureCodeMeasurementPreconfiguration
}

func (x *MeasurementPreconfigurationRefuse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *MeasurementPreconfigurationRefuse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementPreconfigurationRefuse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *MeasurementPreconfigurationRefuse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementPreconfigurationRefuse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
