...
type MeasurementAbort struct {
	NRPPATransactionID int64             // mandatory
	LMFMeasurementID   *ie.MeasurementID // mandatory
	RANMeasurementID   *ie.MeasurementID // mandatory,
}

func (x *MeasurementAbort) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *MeasurementAbort) ProcedureCode() int64 {
	return ProcedureCodeMeasurementAbort
}

func (x *MeasurementAbort) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *MeasurementAbort) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementAbort (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *MeasurementAbort) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementAbort) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
