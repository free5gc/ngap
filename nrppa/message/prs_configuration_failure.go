...
type PRSConfigurationFailure struct {
	NRPPATransactionID     int64                      // mandatory
	Cause                  *ie.Cause                  // mandatory
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional,
}

func (x *PRSConfigurationFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *PRSConfigurationFailure) ProcedureCode() int64 {
	return ProcedureCodePRSConfigurationExchange
}

func (x *PRSConfigurationFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PRSConfigurationFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: PRSConfigurationFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *PRSConfigurationFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PRSConfigurationFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
