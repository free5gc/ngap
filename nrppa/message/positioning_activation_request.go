...
type PositioningActivationRequest struct {
	...
	SRSType            *ie.SRSType          // mandatory
	ActivationTime     *ie.RelativeTime1900 // optional
}

func (x *PositioningActivationRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PositioningActivationRequest) ProcedureCode() int64 {
	return ProcedureCodePositioningActivation
}

func (x *PositioningActivationRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PositioningActivationRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: PositioningActivationRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.SRSType != nil {
		err = x.SRSType.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDSRSType},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field SRSType is missing")
	}

	// IE Field 2 (optional)
	if x.ActivationTime != nil {
		err = x.ActivationTime.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDActivationTime},
...
func (x *PositioningActivationRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PositioningActivationRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDSRSType {
			// check if ie is duplicated
			if x.SRSType != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: SRSType")
			...
			x.SRSType = &ie.SRSType{}
			err = x.SRSType.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDActivationTime {
			// check if ie is duplicated
			if x.ActivationTime != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: ActivationTime")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.ActivationTime = &ie.RelativeTime1900{}
			err = x.ActivationTime.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.SRSType == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDSRSType, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE SRSType is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
