...
type MeasurementActivation struct {
	NRPPATransactionID      int64                       // mandatory
	RequestType             *ie.RequestType             // mandatory
	PRSMeasurementsInfoList *ie.PRSMeasurementsInfoList // optional,
}

func (x *MeasurementActivation) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *MeasurementActivation) ProcedureCode() int64 {
	return ProcedureCodeMeasurementActivation
}

func (x *MeasurementActivation) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *MeasurementActivation) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementActivation (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.RequestType != nil {
		err = x.RequestType.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDRequestType},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field RequestType is missing")
	}

	// IE Field 2 (optional,)
	if x.PRSMeasurementsInfoList != nil {
		err = x.PRSMeasurementsInfoList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPRSMeasurementsInfoList},
...
func (x *MeasurementActivation) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementActivation) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDRequestType {
			// check if ie is duplicated
			if x.RequestType != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: RequestType")
			...
			x.RequestType = &ie.RequestType{}
			err = x.RequestType.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDPRSMeasurementsInfoList {
			// check if ie is duplicated
			if x.PRSMeasurementsInfoList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PRSMeasurementsInfoList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PRSMeasurementsInfoList = &ie.PRSMeasurementsInfoList{}
			err = x.PRSMeasurementsInfoList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.RequestType == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDRequestType, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE RequestType is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
