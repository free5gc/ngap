...
type MeasurementResponse struct {
	...
	TRPMeasurementResponseList *ie.TRPMeasurementResponseList // optional
	CriticalityDiagnostics     *ie.CriticalityDiagnostics     // optional,
}

func (x *MeasurementResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *MeasurementResponse) ProcedureCode() int64 {
	return ProcedureCodeMeasurement
}

func (x *MeasurementResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *MeasurementResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 3 (optional)
	if x.TRPMeasurementResponseList != nil {
		err = x.TRPMeasurementResponseList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTRPMeasurementResponseList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *MeasurementResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDTRPMeasurementResponseList {
			// check if ie is duplicated
			if x.TRPMeasurementResponseList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TRPMeasurementResponseList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TRPMeasurementResponseList = &ie.TRPMeasurementResponseList{}
			err = x.TRPMeasurementResponseList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
