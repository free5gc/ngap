...
func (x *OTDOAInformationFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *OTDOAInformationFailure) ProcedureCode() int64 {
	return ProcedureCodeOTDOAInformationExchange
}

func (x *OTDOAInformationFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *OTDOAInformationFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: OTDOAInformationFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *OTDOAInformationFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (OTDOAInformationFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
