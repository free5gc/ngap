...
func (x *ECIDMeasurementInitiationFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *ECIDMeasurementInitiationFailure) ProcedureCode() int64 {
	return ProcedureCodeECIDMeasurementInitiation
}

func (x *ECIDMeasurementInitiationFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *ECIDMeasurementInitiationFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: ECIDMeasurementInitiationFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *ECIDMeasurementInitiationFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (ECIDMeasurementInitiationFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
