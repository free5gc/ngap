...
type MeasurementPreconfigurationConfirm struct {
	NRPPATransactionID     int64                      // mandatory
	PreconfigurationResult *ie.PreconfigurationResult // mandatory
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional
}

func (x *MeasurementPreconfigurationConfirm) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *MeasurementPreconfigurationConfirm) ProcedureCode() int64 {
	return ProcedureCodeMeasurementPreconfiguration
}

func (x *MeasurementPreconfigurationConfirm) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *MeasurementPreconfigurationConfirm) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 32767
	...
	// open type value: MeasurementPreconfigurationConfirm (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PreconfigurationResult != nil {
		err = x.PreconfigurationResult.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPreconfigurationResult},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PreconfigurationResult is missing")
	}
...
func (x *MeasurementPreconfigurationConfirm) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MeasurementPreconfigurationConfirm) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDPreconfigurationResult {
			// check if ie is duplicated
			if x.PreconfigurationResult != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PreconfigurationResult")
			...
			x.PreconfigurationResult = &ie.PreconfigurationResult{}
			err = x.PreconfigurationResult.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// PreconfigurationResult is mandatory but may be nil (ignored)
