...
var _ ProtocolIE = &OtherRATMeasurementResult{}

type OtherRATMeasurementResult struct {
	List []OtherRATMeasuredResultsValue // sizeLB:1,sizeUB:64
}

func (x *OtherRATMeasurementResult) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *OtherRATMeasurementResult) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []OtherRATMeasuredResultsValue{}
	for i := 0; i < int(numElementsList); i++ {
		var val OtherRATMeasuredResultsValue
...
func (x *OtherRATMeasurementResult) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *OtherRATMeasurementResult) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
