...
var _ ProtocolIE = &TRPMeasurementQuantities{}

type TRPMeasurementQuantities struct {
	List []TRPMeasurementQuantitiesListItem // valueExt,sizeLB:1,sizeUB:16384
}

func (x *TRPMeasurementQuantities) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPMeasurementQuantities) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16384
	...
	x.List = []TRPMeasurementQuantitiesListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPMeasurementQuantitiesListItem
...
func (x *TRPMeasurementQuantities) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TRPMeasurementQuantities) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
