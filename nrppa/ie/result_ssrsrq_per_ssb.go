...
type ResultSSRSRQPerSSB struct {
	List []ResultSSRSRQPerSSBItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *ResultSSRSRQPerSSB) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultSSRSRQPerSSB) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []ResultSSRSRQPerSSBItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ResultSSRSRQPerSSBItem
