...
var _ ProtocolIE = &WLANMeasurementQuantities{}

type WLANMeasurementQuantities struct {
	List []ProtocolIESingleContainerWLANMeasurementQuantitiesItemIEs // sizeLB:0,sizeUB:64
}

func (x *WLANMeasurementQuantities) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WLANMeasurementQuantities) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 0, 64
	...
	x.List = []ProtocolIESingleContainerWLANMeasurementQuantitiesItemIEs{}
	for i := 0; i < int(numElementsList); i++ {
		var val ProtocolIESingleContainerWLANMeasurementQuantitiesItemIEs
...
func (x *WLANMeasurementQuantities) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WLANMeasurementQuantities) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
