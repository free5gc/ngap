...
var _ ProtocolIE = &TRPMeasurementUpdateList{}

type TRPMeasurementUpdateList struct {
	List []TRPMeasurementUpdateItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *TRPMeasurementUpdateList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPMeasurementUpdateList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []TRPMeasurementUpdateItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPMeasurementUpdateItem
...
func (x *TRPMeasurementUpdateList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TRPMeasurementUpdateList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
