...
type AdditionalPathList struct {
	List []AdditionalPathListItem // valueExt,sizeLB:1,sizeUB:2
}

func (x *AdditionalPathList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AdditionalPathList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 2
	...
	x.List = []AdditionalPathListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val AdditionalPathListItem
