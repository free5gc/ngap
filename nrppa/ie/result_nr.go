...
var _ ProtocolIE = &ResultNR{}

type ResultNR struct {
	List []ResultNRItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *ResultNR) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultNR) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *ResultNR) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *ResultNR) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
