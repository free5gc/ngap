...
var _ ProtocolIE = &PRSTRPList{}

type PRSTRPList struct {
	List []PRSTRPItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *PRSTRPList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PRSTRPList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *PRSTRPList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PRSTRPList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
