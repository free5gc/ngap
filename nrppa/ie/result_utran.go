...
type ResultUTRAN struct {
	List []ResultUTRANItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *ResultUTRAN) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultUTRAN) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []ResultUTRANItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ResultUTRANItem
