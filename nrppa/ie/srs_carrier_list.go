...
type SRSCarrierList struct {
	List []SRSCarrierListItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *SRSCarrierList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SRSCarrierList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []SRSCarrierListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val SRSCarrierListItem
