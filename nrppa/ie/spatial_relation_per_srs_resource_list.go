...
type SpatialRelationPerSRSResourceList struct {
	List []SpatialRelationPerSRSResourceItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *SpatialRelationPerSRSResourceList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SpatialRelationPerSRSResourceList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []SpatialRelationPerSRSResourceItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val SpatialRelationPerSRSResourceItem
