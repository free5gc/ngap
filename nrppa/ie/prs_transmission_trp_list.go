...
var _ ProtocolIE = &PRSTransmissionTRPList{}

type PRSTransmissionTRPList struct {
	List []PRSTransmissionTRPItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *PRSTransmissionTRPList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PRSTransmissionTRPList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 65535
	...
	x.List = []PRSTransmissionTRPItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val PRSTransmissionTRPItem
...
func (x *PRSTransmissionTRPList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PRSTransmissionTRPList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
