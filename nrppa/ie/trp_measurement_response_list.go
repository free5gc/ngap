...
var _ ProtocolIE = &TRPMeasurementResponseList{}

type TRPMeasurementResponseList struct {
	List []TRPMeasurementResponseItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *TRPMeasurementResponseList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPMeasurementResponseList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []TRPMeasurementResponseItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPMeasurementResponseItem
...
func (x *TRPMeasurementResponseList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TRPMeasurementResponseList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
