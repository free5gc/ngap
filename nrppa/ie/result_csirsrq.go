...
var _ ProtocolIE = &ResultCSIRSRQ{}

type ResultCSIRSRQ struct {
	List []ResultCSIRSRQItem // valueExt,sizeLB:1,sizeUB:9
}

func (x *ResultCSIRSRQ) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultCSIRSRQ) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 9
	...
	x.List = []ResultCSIRSRQItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ResultCSIRSRQItem
...
func (x *ResultCSIRSRQ) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *ResultCSIRSRQ) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
