...
type TrpMeasurementResult struct {
	List []TrpMeasurementResultItem // valueExt,sizeLB:1,sizeUB:16384
}

func (x *TrpMeasurementResult) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TrpMeasurementResult) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16384
	...
	x.List = []TrpMeasurementResultItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TrpMeasurementResultItem
