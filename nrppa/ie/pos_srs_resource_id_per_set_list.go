...
type PosSRSResourceIDPerSetList struct {
	List []SRSPosResourceID // sizeLB:1,sizeUB:16
}

func (x *PosSRSResourceIDPerSetList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PosSRSResourceIDPerSetList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []SRSPosResourceID{}
	for i := 0; i < int(numElementsList); i++ {
		var val SRSPosResourceID
