...
var _ ProtocolIE = &TRPPRSInformationList{}

type TRPPRSInformationList struct {
	List []TRPPRSInformationListItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *TRPPRSInformationList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPPRSInformationList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []TRPPRSInformationListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPPRSInformationListItem
...
func (x *TRPPRSInformationList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TRPPRSInformationList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
