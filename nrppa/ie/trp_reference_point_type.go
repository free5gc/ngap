...
type TRPReferencePointType struct {
	Choice TRPReferencePointTypeAlt
}

type TRPReferencePointTypeAlt interface {
	TRPReferencePointTypeAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 RelativeGeodeticLocation) TRPReferencePointTypeAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 RelativeCartesianLocation) TRPReferencePointTypeAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerTRPReferencePointTypeExtIEs) TRPReferencePointTypeAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *TRPReferencePointType) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.TRPReferencePointTypeAltIndex()
...
func (x *TRPReferencePointType) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(ProtocolIESingleContainerTRPReferencePointTypeExtIEs)
	} else {
