...
type RequestedDLPRSResourceSetList struct {
	List []RequestedDLPRSResourceSetItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *RequestedDLPRSResourceSetList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *RequestedDLPRSResourceSetList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []RequestedDLPRSResourceSetItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val RequestedDLPRSResourceSetItem
