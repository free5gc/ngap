...
var _ ProtocolIE = &UEMeasurementID{}

type UEMeasurementID struct {
	Value int64 // valueExt,valueLB:1,valueUB:15
}

func (x *UEMeasurementID) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *UEMeasurementID) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *UEMeasurementID) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *UEMeasurementID) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
