...
type SpatialRelationPos struct {
	Choice SpatialRelationPosAlt
}

type SpatialRelationPosAlt interface {
	SpatialRelationPosAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 SSB) SpatialRelationPosAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 PRSInformationPos) SpatialRelationPosAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerSpatialInformationPosExtIEs) SpatialRelationPosAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *SpatialRelationPos) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.SpatialRelationPosAltIndex()
...
func (x *SpatialRelationPos) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(PRSInformationPos)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerSpatialInformationPosExtIEs)
	} else {
