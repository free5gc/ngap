...
type PosSRSResourceList struct {
	List []PosSRSResourceItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *PosSRSResourceList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PosSRSResourceList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []PosSRSResourceItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val PosSRSResourceItem
