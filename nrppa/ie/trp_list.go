...
var _ ProtocolIE = &TRPList{}

type TRPList struct {
	List []TRPItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *TRPList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *TRPList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TRPList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
