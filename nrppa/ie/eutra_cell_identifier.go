...
type EUTRACellIdentifier struct {
	Value aper.BitString // sizeLB:28,sizeUB:28
}

func (x *EUTRACellIdentifier) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *EUTRACellIdentifier) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
