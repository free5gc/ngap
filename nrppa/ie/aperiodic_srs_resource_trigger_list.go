...
type AperiodicSRSResourceTriggerList struct {
	List []AperiodicSRSResourceTrigger // sizeLB:1,sizeUB:3
}

func (x *AperiodicSRSResourceTriggerList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AperiodicSRSResourceTriggerList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 3
	...
	x.List = []AperiodicSRSResourceTrigger{}
	for i := 0; i < int(numElementsList); i++ {
		var val AperiodicSRSResourceTrigger
