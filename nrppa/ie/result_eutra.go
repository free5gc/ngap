...
var _ ProtocolIE = &ResultEUTRA{}

type ResultEUTRA struct {
	List []ResultEUTRAItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *ResultEUTRA) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultEUTRA) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []ResultEUTRAItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ResultEUTRAItem
...
func (x *ResultEUTRA) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *ResultEUTRA) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
