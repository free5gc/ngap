...
var _ ProtocolIE = &SRSType{}

type SRSType struct {
	Choice SRSTypeAlt
}

type SRSTypeAlt interface {
	SRSTypeAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 SemipersistentSRS) SRSTypeAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 AperiodicSRS) SRSTypeAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerSRSTypeExtIEs) SRSTypeAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *SRSType) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.SRSTypeAltIndex()
...
func (x *SRSType) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(SemipersistentSRS)
	} else if option_idx == 1 {
		x.Choice = new(AperiodicSRS)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerSRSTypeExtIEs)
	} else {
...
func (x *SRSType) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *SRSType) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
