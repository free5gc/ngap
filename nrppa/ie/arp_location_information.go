...
type ARPLocationInformation struct {
	List []ARPLocationInformationItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *ARPLocationInformation) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ARPLocationInformation) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []ARPLocationInformationItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ARPLocationInformationItem
