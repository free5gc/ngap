...
var _ ProtocolIE = &Broadcast{}

const ( /* Enum Type */
	BroadcastPresentStart aper.Enumerated = 0
	BroadcastPresentStop  aper.Enumerated = 1
)
...
func (x *Broadcast) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *Broadcast) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *Broadcast) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *Broadcast) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
