...
type TRPInformationTypeResponseList struct {
	List []TRPInformationTypeResponseItem // sizeLB:1,sizeUB:64
}

func (x *TRPInformationTypeResponseList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPInformationTypeResponseList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []TRPInformationTypeResponseItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPInformationTypeResponseItem
