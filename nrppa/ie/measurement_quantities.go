...
var _ ProtocolIE = &MeasurementQuantities{}

type MeasurementQuantities struct {
	List []ProtocolIESingleContainerMeasurementQuantitiesItemIEs // sizeLB:1,sizeUB:64
}

func (x *MeasurementQuantities) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MeasurementQuantities) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []ProtocolIESingleContainerMeasurementQuantitiesItemIEs{}
	for i := 0; i < int(numElementsList); i++ {
		var val ProtocolIESingleContainerMeasurementQuantitiesItemIEs
...
func (x *MeasurementQuantities) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MeasurementQuantities) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
