...
type PeriodicityList struct {
	List []PeriodicityItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *PeriodicityList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PeriodicityList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []PeriodicityItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val PeriodicityItem
