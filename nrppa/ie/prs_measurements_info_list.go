...
var _ ProtocolIE = &PRSMeasurementsInfoList{}

type PRSMeasurementsInfoList struct {
	List []PRSMeasurementsInfoListItem // valueExt,sizeLB:1,sizeUB:4
}

func (x *PRSMeasurementsInfoList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PRSMeasurementsInfoList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 4
	...
	x.List = []PRSMeasurementsInfoListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val PRSMeasurementsInfoListItem
...
func (x *PRSMeasurementsInfoList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PRSMeasurementsInfoList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
