...
type ARPLocationType struct {
	Choice ARPLocationTypeAlt
}

type ARPLocationTypeAlt interface {
	ARPLocationTypeAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 RelativeGeodeticLocation) ARPLocationTypeAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 RelativeCartesianLocation) ARPLocationTypeAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerARPLocationTypeExtIEs) ARPLocationTypeAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *ARPLocationType) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.ARPLocationTypeAltIndex()
...
func (x *ARPLocationType) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(ProtocolIESingleContainerARPLocationTypeExtIEs)
	} else {
