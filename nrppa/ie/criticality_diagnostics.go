...
func (x *CriticalityDiagnostics) Write(pd *aper.PerBitData) error {
	...
	if x.NrppatransactionID != nil {
		// Write struct defined elsewhere (Pointer)
		err = x.NrppatransactionID.Write(pd)
		if err != nil {
			return errors.Wrap(err, "NrppatransactionID marshal failed")
		}
	}
...
func (x *CriticalityDiagnostics) Read(pd *aper.PerBitData) error {
	...
	// Read optPresentFlag
	CriticalityDiagnosticsOptPresentFlag := make([]bool, 6)
	...
	if CriticalityDiagnosticsOptPresentFlag[3] {
		// Read struct defined elsewhere (Pointer)
		x.NrppatransactionID = new(NRPPATransactionID)
		err = x.NrppatransactionID.Read(pd)
		if err != nil {
			return errors.Wrap(err, "decode NrppatransactionID error")
		}
	}
	...
	if CriticalityDiagnosticsOptPresentFlag[5] {
		// Read struct defined elsewhere (Pointer)
