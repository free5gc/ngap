...
type RequestedDLPRSResourceList struct {
	List []RequestedDLPRSResourceItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *RequestedDLPRSResourceList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *RequestedDLPRSResourceList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []RequestedDLPRSResourceItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val RequestedDLPRSResourceItem
