...
type MultipleULAoAList struct {
	List []MultipleULAoAItem // sizeLB:1,sizeUB:8
}

func (x *MultipleULAoAList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MultipleULAoAList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []MultipleULAoAItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val MultipleULAoAItem
