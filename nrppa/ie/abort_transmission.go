...
var _ ProtocolIE = &AbortTransmission{}

type AbortTransmission struct {
	Choice AbortTransmissionAlt
}

type AbortTransmissionAlt interface {
	AbortTransmissionAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 SRSResourceSetID) AbortTransmissionAltIndex() int64 {
	return int64(0)
}

// Choice is an aper-defined/built-in type, complete interface implementation is required
type ReleaseALLForAbortTransmission struct {
	Value aper.NULL
}

func (alt1 *ReleaseALLForAbortTransmission) AbortTransmissionAltIndex() int64 {
	return int64(1)
}

func (x *ReleaseALLForAbortTransmission) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ReleaseALLForAbortTransmission) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerAbortTransmissionExtIEs) AbortTransmissionAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *AbortTransmission) Write(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	var option_idx int64 = x.Choice.AbortTransmissionAltIndex()
...
func (x *AbortTransmission) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(SRSResourceSetID)
	} else if option_idx == 1 {
		x.Choice = new(ReleaseALLForAbortTransmission)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerAbortTransmissionExtIEs)
	} else {
...
func (x *AbortTransmission) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *AbortTransmission) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
