...
var _ ProtocolIE = &PositioningBroadcastCells{}

type PositioningBroadcastCells struct {
	List []NGRANCGI // valueExt,sizeLB:1,sizeUB:16384
}

func (x *PositioningBroadcastCells) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PositioningBroadcastCells) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *PositioningBroadcastCells) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PositioningBroadcastCells) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
