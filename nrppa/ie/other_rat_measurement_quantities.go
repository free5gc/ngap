...
var _ ProtocolIE = &OtherRATMeasurementQuantities{}

type OtherRATMeasurementQuantities struct {
	List []ProtocolIESingleContainerOtherRATMeasurementQuantitiesItemIEs // sizeLB:0,sizeUB:64
}

func (x *OtherRATMeasurementQuantities) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *OtherRATMeasurementQuantities) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 0, 64
	...
	x.List = []ProtocolIESingleContainerOtherRATMeasurementQuantitiesItemIEs{}
	for i := 0; i < int(numElementsList); i++ {
		var val ProtocolIESingleContainerOtherRATMeasurementQuantitiesItemIEs
...
func (x *OtherRATMeasurementQuantities) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *OtherRATMeasurementQuantities) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
