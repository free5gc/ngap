...
type PRSTransmissionOffPerResourceSet struct {
	List []PRSTransmissionOffPerResourceSetItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *PRSTransmissionOffPerResourceSet) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PRSTransmissionOffPerResourceSet) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []PRSTransmissionOffPerResourceSetItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val PRSTransmissionOffPerResourceSetItem
