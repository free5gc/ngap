...
type SRSResourceList struct {
	List []SRSResource // valueExt,sizeLB:1,sizeUB:64
}

func (x *SRSResourceList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SRSResourceList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
