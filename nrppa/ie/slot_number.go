...
var _ ProtocolIE = &SlotNumber{}

type SlotNumber struct {
	Value int64 // valueLB:0,valueUB:79
}

func (x *SlotNumber) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SlotNumber) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *SlotNumber) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *SlotNumber) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
