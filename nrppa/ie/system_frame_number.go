...
var _ ProtocolIE = &SystemFrameNumber{}

type SystemFrameNumber struct {
	Value int64 // valueLB:0,valueUB:1023
}

func (x *SystemFrameNumber) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SystemFrameNumber) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *SystemFrameNumber) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *SystemFrameNumber) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
