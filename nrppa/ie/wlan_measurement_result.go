...
var _ ProtocolIE = &WLANMeasurementResult{}

type WLANMeasurementResult struct {
	List []WLANMeasurementResultItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *WLANMeasurementResult) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WLANMeasurementResult) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []WLANMeasurementResultItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val WLANMeasurementResultItem
...
func (x *WLANMeasurementResult) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WLANMeasurementResult) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
