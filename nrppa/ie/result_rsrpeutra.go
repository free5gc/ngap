...
type ResultRSRPEUTRA struct {
	List []ResultRSRPEUTRAItem // valueExt,sizeLB:1,sizeUB:9
}

func (x *ResultRSRPEUTRA) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ResultRSRPEUTRA) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 9
	...
	x.List = []ResultRSRPEUTRAItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val ResultRSRPEUTRAItem
