...
type OTDOACellInformation struct {
	List []OTDOACellInformationItem // sizeLB:1,sizeUB:63
}

func (x *OTDOACellInformation) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *OTDOACellInformation) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 63
	...
	x.List = []OTDOACellInformationItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val OTDOACellInformationItem
