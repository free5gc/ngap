...
type TRPBeamAntennaAngles struct {
	List []TRPBeamAntennaAnglesListItem // valueExt,sizeLB:1,sizeUB:3600
}

func (x *TRPBeamAntennaAngles) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TRPBeamAntennaAngles) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 3600
	...
	x.List = []TRPBeamAntennaAnglesListItem{}
	for i := 0; i < int(numElementsList); i++ {
		var val TRPBeamAntennaAnglesListItem
