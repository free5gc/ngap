...
type SliceSupportList struct {
	List []SliceSupportItem // valueExt,sizeLB:1,sizeUB:1024
}

func (x *SliceSupportList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SliceSupportList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
