...
type ExcessPacketDelayThresholdConfiguration struct {
	List []ExcessPacketDelayThresholdItem // valueExt,sizeLB:1,sizeUB:255
}

func (x *ExcessPacketDelayThresholdConfiguration) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ExcessPacketDelayThresholdConfiguration) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 255
	...
	x.List = []ExcessPacketDelayThresholdItem{}
	for i := 0; i < int(numElements); i++ {
		var val ExcessPacketDelayThresholdItem
