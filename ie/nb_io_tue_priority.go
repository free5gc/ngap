...
var _ ProtocolIE = &NBIoTUEPriority{}

type NBIoTUEPriority struct {
	Value int64 // valueExt,valueLB:0,valueUB:255
}

func (x *NBIoTUEPriority) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NBIoTUEPriority) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *NBIoTUEPriority) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *NBIoTUEPriority) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
