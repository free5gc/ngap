...
var _ ProtocolIE = &QosFlowAddOrModifyRequestList{}

type QosFlowAddOrModifyRequestList struct {
	List []QosFlowAddOrModifyRequestItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowAddOrModifyRequestList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowAddOrModifyRequestList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowAddOrModifyRequestItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowAddOrModifyRequestItem
...
func (x *QosFlowAddOrModifyRequestList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *QosFlowAddOrModifyRequestList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
