...
type CNTypeRestrictionsForEquivalent struct {
	List []CNTypeRestrictionsForEquivalentItem // valueExt,sizeLB:1,sizeUB:15
}

func (x *CNTypeRestrictionsForEquivalent) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *CNTypeRestrictionsForEquivalent) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 15
	...
	x.List = []CNTypeRestrictionsForEquivalentItem{}
	for i := 0; i < int(numElements); i++ {
		var val CNTypeRestrictionsForEquivalentItem
