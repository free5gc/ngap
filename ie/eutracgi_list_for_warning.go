...
type EUTRACGIListForWarning struct {
	List []EUTRACGI // valueExt,sizeLB:1,sizeUB:65535
}

func (x *EUTRACGIListForWarning) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *EUTRACGIListForWarning) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
