...
type AssociatedMBSQosFlowSetuporModifyRequestList struct {
	List []AssociatedMBSQosFlowSetuporModifyRequestItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *AssociatedMBSQosFlowSetuporModifyRequestList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AssociatedMBSQosFlowSetuporModifyRequestList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []AssociatedMBSQosFlowSetuporModifyRequestItem{}
	for i := 0; i < int(numElements); i++ {
		var val AssociatedMBSQosFlowSetuporModifyRequestItem
