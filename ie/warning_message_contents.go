...
var _ ProtocolIE = &WarningMessageContents{}

type WarningMessageContents struct {
	Value aper.OctetString // sizeLB:1,sizeUB:9600
}

func (x *WarningMessageContents) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WarningMessageContents) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *WarningMessageContents) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WarningMessageContents) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
