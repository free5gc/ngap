...
type AreaOfInterestList struct {
	List []AreaOfInterestItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *AreaOfInterestList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AreaOfInterestList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []AreaOfInterestItem{}
	for i := 0; i < int(numElements); i++ {
		var val AreaOfInterestItem
