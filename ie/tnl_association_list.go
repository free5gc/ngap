...
var _ ProtocolIE = &TNLAssociationList{}

type TNLAssociationList struct {
	List []TNLAssociationItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *TNLAssociationList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TNLAssociationList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []TNLAssociationItem{}
	for i := 0; i < int(numElements); i++ {
		var val TNLAssociationItem
...
func (x *TNLAssociationList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TNLAssociationList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
