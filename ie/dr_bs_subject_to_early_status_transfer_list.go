...
type DRBsSubjectToEarlyStatusTransferList struct {
	List []DRBsSubjectToEarlyStatusTransferItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *DRBsSubjectToEarlyStatusTransferList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *DRBsSubjectToEarlyStatusTransferList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []DRBsSubjectToEarlyStatusTransferItem{}
	for i := 0; i < int(numElements); i++ {
		var val DRBsSubjectToEarlyStatusTransferItem
