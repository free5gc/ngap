...
type UPTransportLayerInformationPairList struct {
	List []UPTransportLayerInformationPairItem // valueExt,sizeLB:1,sizeUB:3
}

func (x *UPTransportLayerInformationPairList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *UPTransportLayerInformationPairList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 3
	...
	x.List = []UPTransportLayerInformationPairItem{}
	for i := 0; i < int(numElements); i++ {
		var val UPTransportLayerInformationPairItem
