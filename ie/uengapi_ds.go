...
var _ ProtocolIE = &UENGAPIDs{}

type UENGAPIDs struct {
	Choice UENGAPIDsAlt
}

type UENGAPIDsAlt interface {
	UENGAPIDsAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 UENGAPIDPair) UENGAPIDsAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 AMFUENGAPID) UENGAPIDsAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerUENGAPIDsExtIEs) UENGAPIDsAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *UENGAPIDs) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.UENGAPIDsAltIndex()
...
func (x *UENGAPIDs) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(UENGAPIDPair)
	} else if option_idx == 1 {
		x.Choice = new(AMFUENGAPID)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerUENGAPIDsExtIEs)
	} else {
...
func (x *UENGAPIDs) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *UENGAPIDs) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
