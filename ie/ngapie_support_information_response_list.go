...
type NGAPIESupportInformationResponseList struct {
	List []NGAPIESupportInformationResponseItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *NGAPIESupportInformationResponseList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NGAPIESupportInformationResponseList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []NGAPIESupportInformationResponseItem{}
	for i := 0; i < int(numElements); i++ {
		var val NGAPIESupportInformationResponseItem
