...
type QoEReferenceList struct {
	List []QoEReference // sizeLB:1,sizeUB:16
}

func (x *QoEReferenceList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QoEReferenceList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
