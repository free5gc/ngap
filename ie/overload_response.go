...
var _ ProtocolIE = &OverloadResponse{}

type OverloadResponse struct {
	Choice OverloadResponseAlt
}

type OverloadResponseAlt interface {
	OverloadResponseAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 OverloadAction) OverloadResponseAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 ProtocolIESingleContainerOverloadResponseExtIEs) OverloadResponseAltIndex() int64 {
	return int64(1)
}

// Choice Type Read/Write Functions

func (x *OverloadResponse) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.OverloadResponseAltIndex()
...
func (x *OverloadResponse) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(OverloadAction)
	} else if option_idx == 1 {
		x.Choice = new(ProtocolIESingleContainerOverloadResponseExtIEs)
	} else {
...
func (x *OverloadResponse) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *OverloadResponse) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
