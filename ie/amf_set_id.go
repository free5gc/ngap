...
var _ ProtocolIE = &AMFSetID{}

type AMFSetID struct {
	Value aper.BitString // sizeLB:10,sizeUB:10
}

func (x *AMFSetID) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AMFSetID) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *AMFSetID) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *AMFSetID) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
