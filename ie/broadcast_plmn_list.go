...
type BroadcastPLMNList struct {
	List []BroadcastPLMNItem // valueExt,sizeLB:1,sizeUB:12
}

func (x *BroadcastPLMNList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *BroadcastPLMNList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 12
	...
	x.List = []BroadcastPLMNItem{}
	for i := 0; i < int(numElements); i++ {
		var val BroadcastPLMNItem
