...
var _ ProtocolIE = &TAIListForRestart{}

type TAIListForRestart struct {
	List []TAI // valueExt,sizeLB:1,sizeUB:2048
}

func (x *TAIListForRestart) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TAIListForRestart) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *TAIListForRestart) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TAIListForRestart) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
