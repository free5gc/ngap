...
type MBSSessionTNLInfo5GCList struct {
	List []MBSSessionTNLInfo5GCItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *MBSSessionTNLInfo5GCList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSSessionTNLInfo5GCList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []MBSSessionTNLInfo5GCItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSSessionTNLInfo5GCItem
