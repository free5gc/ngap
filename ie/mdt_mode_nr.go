...
type MDTModeNrAlt interface {
	MDTModeNrAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 ImmediateMDTNr) MDTModeNrAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 LoggedMDTNr) MDTModeNrAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerMDTModeNrExtIEs) MDTModeNrAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *MDTModeNr) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.MDTModeNrAltIndex()
...
func (x *MDTModeNr) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(ImmediateMDTNr)
	} else if option_idx == 1 {
		x.Choice = new(LoggedMDTNr)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerMDTModeNrExtIEs)
	} else {
