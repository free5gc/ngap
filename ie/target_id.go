...
var _ ProtocolIE = &TargetID{}

type TargetID struct {
	Choice TargetIDAlt
}

type TargetIDAlt interface {
	TargetIDAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 TargetRANNodeID) TargetIDAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 TargeteNBID) TargetIDAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerTargetIDExtIEs) TargetIDAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *TargetID) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.TargetIDAltIndex()
...
func (x *TargetID) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(TargetRANNodeID)
	} else if option_idx == 1 {
		x.Choice = new(TargeteNBID)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerTargetIDExtIEs)
	} else {
...
func (x *TargetID) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TargetID) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
