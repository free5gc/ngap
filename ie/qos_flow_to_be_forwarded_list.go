...
type QosFlowToBeForwardedList struct {
	List []QosFlowToBeForwardedItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowToBeForwardedList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowToBeForwardedList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowToBeForwardedItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowToBeForwardedItem
