...
type DRBStatusUL struct {
	Choice DRBStatusULAlt
}

type DRBStatusULAlt interface {
	DRBStatusULAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 DRBStatusUL12) DRBStatusULAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 DRBStatusUL18) DRBStatusULAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerDRBStatusULExtIEs) DRBStatusULAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *DRBStatusUL) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.DRBStatusULAltIndex()
...
func (x *DRBStatusUL) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(DRBStatusUL12)
	} else if option_idx == 1 {
		x.Choice = new(DRBStatusUL18)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerDRBStatusULExtIEs)
	} else {
