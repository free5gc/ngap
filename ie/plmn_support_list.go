...
var _ ProtocolIE = &PLMNSupportList{}

type PLMNSupportList struct {
	List []PLMNSupportItem // valueExt,sizeLB:1,sizeUB:12
}

func (x *PLMNSupportList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PLMNSupportList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 12
	...
	x.List = []PLMNSupportItem{}
	for i := 0; i < int(numElements); i++ {
		var val PLMNSupportItem
...
func (x *PLMNSupportList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PLMNSupportList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
