...
type MBSServiceAreaTAIList struct {
	List []TAI // valueExt,sizeLB:1,sizeUB:1024
}

func (x *MBSServiceAreaTAIList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSServiceAreaTAIList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
