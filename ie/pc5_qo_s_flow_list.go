...
type PC5QoSFlowList struct {
	List []PC5QoSFlowItem // valueExt,sizeLB:1,sizeUB:2048
}

func (x *PC5QoSFlowList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PC5QoSFlowList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 2048
	...
	x.List = []PC5QoSFlowItem{}
	for i := 0; i < int(numElements); i++ {
		var val PC5QoSFlowItem
