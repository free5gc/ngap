...
var _ ProtocolIE = &MaskedIMEISV{}

type MaskedIMEISV struct {
	Value aper.BitString // sizeLB:64,sizeUB:64
}

func (x *MaskedIMEISV) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MaskedIMEISV) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *MaskedIMEISV) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MaskedIMEISV) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
