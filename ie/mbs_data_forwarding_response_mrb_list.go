...
type MBSDataForwardingResponseMRBList struct {
	List []MBSDataForwardingResponseMRBItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *MBSDataForwardingResponseMRBList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSDataForwardingResponseMRBList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []MBSDataForwardingResponseMRBItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSDataForwardingResponseMRBItem
