...
type QosFlowNotifyList struct {
	List []QosFlowNotifyItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowNotifyList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowNotifyList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowNotifyItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowNotifyItem
