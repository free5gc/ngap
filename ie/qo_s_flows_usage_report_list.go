...
type QoSFlowsUsageReportList struct {
	List []QoSFlowsUsageReportItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QoSFlowsUsageReportList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QoSFlowsUsageReportList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QoSFlowsUsageReportItem{}
	for i := 0; i < int(numElements); i++ {
		var val QoSFlowsUsageReportItem
