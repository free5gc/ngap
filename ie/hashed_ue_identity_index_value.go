...
type HashedUEIdentityIndexValue struct {
	Value aper.BitString // sizeExt,sizeLB:13,sizeUB:13
}

func (x *HashedUEIdentityIndexValue) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *HashedUEIdentityIndexValue) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
