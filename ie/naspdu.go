...
var _ ProtocolIE = &NASPDU{}

type NASPDU struct {
	Value aper.OctetString
}

func (x *NASPDU) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NASPDU) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *NASPDU) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *NASPDU) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
