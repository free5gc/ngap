...
type CancelledCellsInTAINR struct {
	List []CancelledCellsInTAINRItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *CancelledCellsInTAINR) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *CancelledCellsInTAINR) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 65535
	...
	x.List = []CancelledCellsInTAINRItem{}
	for i := 0; i < int(numElements); i++ {
		var val CancelledCellsInTAINRItem
