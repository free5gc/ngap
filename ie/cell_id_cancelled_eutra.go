...
type CellIDCancelledEUTRA struct {
	List []CellIDCancelledEUTRAItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *CellIDCancelledEUTRA) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *CellIDCancelledEUTRA) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 65535
	...
	x.List = []CellIDCancelledEUTRAItem{}
	for i := 0; i < int(numElements); i++ {
		var val CellIDCancelledEUTRAItem
