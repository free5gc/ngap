...
var _ ProtocolIE = &WarningSecurityInfo{}

type WarningSecurityInfo struct {
	Value aper.OctetString // sizeLB:50,sizeUB:50
}

func (x *WarningSecurityInfo) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WarningSecurityInfo) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *WarningSecurityInfo) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WarningSecurityInfo) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
