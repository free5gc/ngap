...
type TargetNSSAI struct {
	List []TargetNSSAIItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *TargetNSSAI) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TargetNSSAI) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []TargetNSSAIItem{}
	for i := 0; i < int(numElements); i++ {
		var val TargetNSSAIItem
