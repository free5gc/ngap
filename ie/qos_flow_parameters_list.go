...
type QosFlowParametersList struct {
	List []QosFlowParametersItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowParametersList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowParametersList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowParametersItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowParametersItem
