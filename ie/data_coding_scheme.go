...
var _ ProtocolIE = &DataCodingScheme{}

type DataCodingScheme struct {
	Value aper.BitString // sizeLB:8,sizeUB:8
}

func (x *DataCodingScheme) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *DataCodingScheme) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *DataCodingScheme) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *DataCodingScheme) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
