...
type EmergencyAreaIDList struct {
	List []EmergencyAreaID // sizeLB:1,sizeUB:65535
}

func (x *EmergencyAreaIDList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *EmergencyAreaIDList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
