...
var _ ProtocolIE = &NumberOfBroadcastsRequested{}

type NumberOfBroadcastsRequested struct {
	Value int64 // valueLB:0,valueUB:65535
}

func (x *NumberOfBroadcastsRequested) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NumberOfBroadcastsRequested) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *NumberOfBroadcastsRequested) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *NumberOfBroadcastsRequested) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
