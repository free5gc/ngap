...
var _ ProtocolIE = &PLMNIdentity{}

type PLMNIdentity struct {
	Value aper.OctetString // sizeLB:3,sizeUB:3
}

func (x *PLMNIdentity) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *PLMNIdentity) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *PLMNIdentity) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *PLMNIdentity) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
