...
var _ ProtocolIE = &UnavailableGUAMIList{}

type UnavailableGUAMIList struct {
	List []UnavailableGUAMIItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *UnavailableGUAMIList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *UnavailableGUAMIList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []UnavailableGUAMIItem{}
	for i := 0; i < int(numElements); i++ {
		var val UnavailableGUAMIItem
...
func (x *UnavailableGUAMIList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *UnavailableGUAMIList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
