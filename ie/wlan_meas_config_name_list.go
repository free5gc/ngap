...
type WLANMeasConfigNameList struct {
	List []WLANMeasConfigNameItem // valueExt,sizeLB:1,sizeUB:4
}

func (x *WLANMeasConfigNameList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WLANMeasConfigNameList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 4
	...
	x.List = []WLANMeasConfigNameItem{}
	for i := 0; i < int(numElements); i++ {
		var val WLANMeasConfigNameItem
