...
type QosFlowAcceptedList struct {
	List []QosFlowAcceptedItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowAcceptedList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowAcceptedList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowAcceptedItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowAcceptedItem
