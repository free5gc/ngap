...
type MBSServiceAreaCellList struct {
	List []NRCGI // valueExt,sizeLB:1,sizeUB:8192
}

func (x *MBSServiceAreaCellList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSServiceAreaCellList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
