...
type AMFPagingTarget struct {
	Choice AMFPagingTargetAlt
}

type AMFPagingTargetAlt interface {
	AMFPagingTargetAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 GlobalRANNodeID) AMFPagingTargetAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 TAI) AMFPagingTargetAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerAMFPagingTargetExtIEs) AMFPagingTargetAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *AMFPagingTarget) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.AMFPagingTargetAltIndex()
...
func (x *AMFPagingTarget) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(GlobalRANNodeID)
	} else if option_idx == 1 {
		x.Choice = new(TAI)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerAMFPagingTargetExtIEs)
	} else {
