...
var _ ProtocolIE = &QosFlowListWithCause{}

type QosFlowListWithCause struct {
	List []QosFlowWithCauseItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *QosFlowListWithCause) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *QosFlowListWithCause) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []QosFlowWithCauseItem{}
	for i := 0; i < int(numElements); i++ {
		var val QosFlowWithCauseItem
...
func (x *QosFlowListWithCause) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *QosFlowListWithCause) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
