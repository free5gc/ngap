...
var _ ProtocolIE = &WarningAreaList{}

type WarningAreaList struct {
	Choice WarningAreaListAlt
}

type WarningAreaListAlt interface {
	WarningAreaListAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 EUTRACGIListForWarning) WarningAreaListAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 NRCGIListForWarning) WarningAreaListAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 TAIListForWarning) WarningAreaListAltIndex() int64 {
	return int64(2)
}

// Choice type and its Read/Write is defined elsewhere
func (alt3 EmergencyAreaIDList) WarningAreaListAltIndex() int64 {
	return int64(3)
}

// Choice type and its Read/Write is defined elsewhere
func (alt4 ProtocolIESingleContainerWarningAreaListExtIEs) WarningAreaListAltIndex() int64 {
	return int64(4)
}

// Choice Type Read/Write Functions

func (x *WarningAreaList) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.WarningAreaListAltIndex()
...
func (x *WarningAreaList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(EUTRACGIListForWarning)
	} else if option_idx == 1 {
		x.Choice = new(NRCGIListForWarning)
	} else if option_idx == 2 {
		x.Choice = new(TAIListForWarning)
	} else if option_idx == 3 {
		x.Choice = new(EmergencyAreaIDList)
	} else if option_idx == 4 {
		x.Choice = new(ProtocolIESingleContainerWarningAreaListExtIEs)
	} else {
...
func (x *WarningAreaList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WarningAreaList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
