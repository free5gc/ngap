...
var _ ProtocolIE = &MBSSessionToReleaseList{}

type MBSSessionToReleaseList struct {
	List []MBSSessionToReleaseItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *MBSSessionToReleaseList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSSessionToReleaseList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []MBSSessionToReleaseItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSSessionToReleaseItem
...
func (x *MBSSessionToReleaseList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MBSSessionToReleaseList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
