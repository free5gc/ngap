...
var _ ProtocolIE = &MBSSessionFSAIDList{}

type MBSSessionFSAIDList struct {
	List []MBSSessionFSAID // sizeLB:1,sizeUB:64
}

func (x *MBSSessionFSAIDList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSSessionFSAIDList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []MBSSessionFSAID{}
	for i := 0; i < int(numElements); i++ {
		var val MBSSessionFSAID
...
func (x *MBSSessionFSAIDList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MBSSessionFSAIDList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
