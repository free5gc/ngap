...
type SliceOverloadList struct {
	List []SliceOverloadItem // valueExt,sizeLB:1,sizeUB:1024
}

func (x *SliceOverloadList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SliceOverloadList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 1024
	...
	x.List = []SliceOverloadItem{}
	for i := 0; i < int(numElements); i++ {
		var val SliceOverloadItem
