...
var _ ProtocolIE = &ServedGUAMIList{}

type ServedGUAMIList struct {
	List []ServedGUAMIItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *ServedGUAMIList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *ServedGUAMIList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []ServedGUAMIItem{}
	for i := 0; i < int(numElements); i++ {
		var val ServedGUAMIItem
...
func (x *ServedGUAMIList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *ServedGUAMIList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
