...
type CellIdListforMDTEUTRA struct {
	List []EUTRACGI // valueExt,sizeLB:1,sizeUB:32
}

func (x *CellIdListforMDTEUTRA) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *CellIdListforMDTEUTRA) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
