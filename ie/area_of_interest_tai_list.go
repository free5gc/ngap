...
type AreaOfInterestTAIList struct {
	List []AreaOfInterestTAIItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *AreaOfInterestTAIList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AreaOfInterestTAIList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []AreaOfInterestTAIItem{}
	for i := 0; i < int(numElements); i++ {
		var val AreaOfInterestTAIItem
