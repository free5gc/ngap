...
var _ ProtocolIE = &TAI{}

type TAI struct {
	PLMNIdentity *PLMNIdentity
	TAC          *TAC
	IEExtensions *ProtocolExtensionContainerTAIExtIEs // optional
}

func (x *TAI) Write(pd *aper.PerBitData) error {
	var err error
	...
	if x.IEExtensions != nil {
		TAIOptPresentFlag = append(TAIOptPresentFlag, true)
	} else {
		TAIOptPresentFlag = append(TAIOptPresentFlag, false)
	}

	err = pd.WriteSequencePreambleBitMap(TAIOptPresentFlag, true)
	if err != nil {
...
func (x *TAI) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read optPresentFlag
	TAIOptPresentFlag := make([]bool, 1)
	err = pd.ReadSequencePreambleBitMap(&TAIOptPresentFlag, true)
	if err != nil {
	...
	if TAIOptPresentFlag[0] {
		// Read struct defined elsewhere (Pointer)
		x.IEExtensions = new(ProtocolExtensionContainerTAIExtIEs)
...
func (x *TAI) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TAI) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
