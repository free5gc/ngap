...
type NPNSupport struct {
	Choice NPNSupportAlt
}

type NPNSupportAlt interface {
	NPNSupportAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 NID) NPNSupportAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 ProtocolIESingleContainerNPNSupportExtIEs) NPNSupportAltIndex() int64 {
	return int64(1)
}

// Choice Type Read/Write Functions

func (x *NPNSupport) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.NPNSupportAltIndex()
...
func (x *NPNSupport) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(ProtocolIESingleContainerNPNSupportExtIEs)
	} else {
