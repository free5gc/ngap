...
type XnExtTLAs struct {
	List []XnExtTLAItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *XnExtTLAs) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *XnExtTLAs) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
