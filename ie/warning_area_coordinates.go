...
var _ ProtocolIE = &WarningAreaCoordinates{}

type WarningAreaCoordinates struct {
	Value aper.OctetString // sizeLB:1,sizeUB:1024
}

func (x *WarningAreaCoordinates) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *WarningAreaCoordinates) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *WarningAreaCoordinates) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *WarningAreaCoordinates) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
