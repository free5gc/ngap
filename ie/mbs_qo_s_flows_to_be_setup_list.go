...
var _ ProtocolIE = &MBSQoSFlowsToBeSetupList{}

type MBSQoSFlowsToBeSetupList struct {
	List []MBSQoSFlowsToBeSetupItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *MBSQoSFlowsToBeSetupList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSQoSFlowsToBeSetupList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []MBSQoSFlowsToBeSetupItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSQoSFlowsToBeSetupItem
...
func (x *MBSQoSFlowsToBeSetupList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MBSQoSFlowsToBeSetupList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
