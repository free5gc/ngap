...
type AreaOfInterestCellList struct {
	List []AreaOfInterestCellItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *AreaOfInterestCellList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AreaOfInterestCellList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []AreaOfInterestCellItem{}
	for i := 0; i < int(numElements); i++ {
		var val AreaOfInterestCellItem
