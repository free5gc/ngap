...
type AssociatedQosFlowList struct {
	List []AssociatedQosFlowItem // valueExt,sizeLB:1,sizeUB:64
}

func (x *AssociatedQosFlowList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AssociatedQosFlowList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 64
	...
	x.List = []AssociatedQosFlowItem{}
	for i := 0; i < int(numElements); i++ {
		var val AssociatedQosFlowItem
