...
var _ ProtocolIE = &GlobalRANNodeID{}

type GlobalRANNodeID struct {
	Choice GlobalRANNodeIDAlt
}

type GlobalRANNodeIDAlt interface {
	GlobalRANNodeIDAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 GlobalGNBID) GlobalRANNodeIDAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 GlobalNgENBID) GlobalRANNodeIDAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 GlobalN3IWFID) GlobalRANNodeIDAltIndex() int64 {
	return int64(2)
}

// Choice type and its Read/Write is defined elsewhere
func (alt3 ProtocolIESingleContainerGlobalRANNodeIDExtIEs) GlobalRANNodeIDAltIndex() int64 {
	return int64(3)
}

// Choice Type Read/Write Functions

func (x *GlobalRANNodeID) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.GlobalRANNodeIDAltIndex()
...
func (x *GlobalRANNodeID) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(GlobalNgENBID)
	} else if option_idx == 2 {
		x.Choice = new(GlobalN3IWFID)
	} else if option_idx == 3 {
		x.Choice = new(ProtocolIESingleContainerGlobalRANNodeIDExtIEs)
	} else {
...
func (x *GlobalRANNodeID) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *GlobalRANNodeID) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
