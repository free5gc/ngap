...
var _ ProtocolIE = &MBSServiceArea{}

type MBSServiceArea struct {
	Choice MBSServiceAreaAlt
}

type MBSServiceAreaAlt interface {
	MBSServiceAreaAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 MBSServiceAreaInformation) MBSServiceAreaAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 MBSServiceAreaInformationList) MBSServiceAreaAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerMBSServiceAreaExtIEs) MBSServiceAreaAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *MBSServiceArea) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.MBSServiceAreaAltIndex()
...
func (x *MBSServiceArea) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(MBSServiceAreaInformation)
	} else if option_idx == 1 {
		x.Choice = new(MBSServiceAreaInformationList)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerMBSServiceAreaExtIEs)
	} else {
...
func (x *MBSServiceArea) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MBSServiceArea) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
