...
type NotificationCellList struct {
	List []NotificationCellItem // valueExt,sizeLB:1,sizeUB:16384
}

func (x *NotificationCellList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NotificationCellList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16384
	...
	x.List = []NotificationCellItem{}
	for i := 0; i < int(numElements); i++ {
		var val NotificationCellItem
