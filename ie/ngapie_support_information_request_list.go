...
type NGAPIESupportInformationRequestList struct {
	List []NGAPIESupportInformationRequestItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *NGAPIESupportInformationRequestList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NGAPIESupportInformationRequestList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []NGAPIESupportInformationRequestItem{}
	for i := 0; i < int(numElements); i++ {
		var val NGAPIESupportInformationRequestItem
