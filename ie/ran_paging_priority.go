...
var _ ProtocolIE = &RANPagingPriority{}

type RANPagingPriority struct {
	Value int64 // valueLB:1,valueUB:256
}

func (x *RANPagingPriority) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *RANPagingPriority) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *RANPagingPriority) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *RANPagingPriority) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
