...
var _ ProtocolIE = &CellIDListForRestart{}

type CellIDListForRestart struct {
	Choice CellIDListForRestartAlt
}

type CellIDListForRestartAlt interface {
	CellIDListForRestartAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 EUTRACGIList) CellIDListForRestartAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 NRCGIList) CellIDListForRestartAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerCellIDListForRestartExtIEs) CellIDListForRestartAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *CellIDListForRestart) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.CellIDListForRestartAltIndex()
...
func (x *CellIDListForRestart) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(EUTRACGIList)
	} else if option_idx == 1 {
		x.Choice = new(NRCGIList)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerCellIDListForRestartExtIEs)
	} else {
...
func (x *CellIDListForRestart) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *CellIDListForRestart) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
