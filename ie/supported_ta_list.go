...
var _ ProtocolIE = &SupportedTAList{}

type SupportedTAList struct {
	List []SupportedTAItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *SupportedTAList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SupportedTAList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []SupportedTAItem{}
	for i := 0; i < int(numElements); i++ {
		var val SupportedTAItem
...
func (x *SupportedTAList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *SupportedTAList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
