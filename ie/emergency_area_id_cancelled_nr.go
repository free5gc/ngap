...
type EmergencyAreaIDCancelledNR struct {
	List []EmergencyAreaIDCancelledNRItem // valueExt,sizeLB:1,sizeUB:65535
}

func (x *EmergencyAreaIDCancelledNR) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *EmergencyAreaIDCancelledNR) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 65535
	...
	x.List = []EmergencyAreaIDCancelledNRItem{}
	for i := 0; i < int(numElements); i++ {
		var val EmergencyAreaIDCancelledNRItem
