...
var _ ProtocolIE = &ResetType{}

type ResetType struct {
	Choice ResetTypeAlt
}

type ResetTypeAlt interface {
	ResetTypeAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 ResetAll) ResetTypeAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 UEAssociatedLogicalNGConnectionList) ResetTypeAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ProtocolIESingleContainerResetTypeExtIEs) ResetTypeAltIndex() int64 {
	return int64(2)
}

// Choice Type Read/Write Functions

func (x *ResetType) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.ResetTypeAltIndex()
...
func (x *ResetType) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(ResetAll)
	} else if option_idx == 1 {
		x.Choice = new(UEAssociatedLogicalNGConnectionList)
	} else if option_idx == 2 {
		x.Choice = new(ProtocolIESingleContainerResetTypeExtIEs)
	} else {
...
func (x *ResetType) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *ResetType) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
