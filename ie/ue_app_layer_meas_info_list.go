...
type UEAppLayerMeasInfoList struct {
	List []UEAppLayerMeasInfoItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *UEAppLayerMeasInfoList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *UEAppLayerMeasInfoList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []UEAppLayerMeasInfoItem{}
	for i := 0; i < int(numElements); i++ {
		var val UEAppLayerMeasInfoItem
