...
var _ ProtocolIE = &IABSupported{}

const ( /* Enum Type */
	IABSupportedPresentTrue aper.Enumerated = 0
)
...
func (x *IABSupported) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *IABSupported) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *IABSupported) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *IABSupported) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
