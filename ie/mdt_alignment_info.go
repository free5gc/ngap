...
type MDTAlignmentInfo struct {
	Choice MDTAlignmentInfoAlt
}

type MDTAlignmentInfoAlt interface {
	MDTAlignmentInfoAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 NGRANTraceID) MDTAlignmentInfoAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 ProtocolIESingleContainerMDTAlignmentInfoExtIEs) MDTAlignmentInfoAltIndex() int64 {
	return int64(1)
}

// Choice Type Read/Write Functions

func (x *MDTAlignmentInfo) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.MDTAlignmentInfoAltIndex()
...
func (x *MDTAlignmentInfo) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(NGRANTraceID)
	} else if option_idx == 1 {
		x.Choice = new(ProtocolIESingleContainerMDTAlignmentInfoExtIEs)
	} else {
