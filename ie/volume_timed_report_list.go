...
type VolumeTimedReportList struct {
	List []VolumeTimedReportItem // valueExt,sizeLB:1,sizeUB:2
}

func (x *VolumeTimedReportList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *VolumeTimedReportList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 2
	...
	x.List = []VolumeTimedReportItem{}
	for i := 0; i < int(numElements); i++ {
		var val VolumeTimedReportItem
