...
var _ ProtocolIE = &Cause{}

type Cause struct {
	Choice CauseAlt
}

type CauseAlt interface {
	CauseAltIndex() int64
...
func (alt0 CauseRadioNetwork) CauseAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 CauseTransport) CauseAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 CauseNas) CauseAltIndex() int64 {
	return int64(2)
}

// Choice type and its Read/Write is defined elsewhere
func (alt3 CauseProtocol) CauseAltIndex() int64 {
	return int64(3)
}

// Choice type and its Read/Write is defined elsewhere
func (alt4 CauseMisc) CauseAltIndex() int64 {
	return int64(4)
}

// Choice type and its Read/Write is defined elsewhere
func (alt5 ProtocolIESingleContainerCauseExtIEs) CauseAltIndex() int64 {
	return int64(5)
}

// Choice Type Read/Write Functions

func (x *Cause) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.CauseAltIndex()
...
func (x *Cause) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	// Read Choice
	if option_idx == 0 {
		x.Choice = new(CauseRadioNetwork)
	} else if option_idx == 1 {
		x.Choice = new(CauseTransport)
	} else if option_idx == 2 {
		x.Choice = new(CauseNas)
	} else if option_idx == 3 {
		x.Choice = new(CauseProtocol)
	} else if option_idx == 4 {
		x.Choice = new(CauseMisc)
	} else if option_idx == 5 {
		x.Choice = new(ProtocolIESingleContainerCauseExtIEs)
	} else {
...
func (x *Cause) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *Cause) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
