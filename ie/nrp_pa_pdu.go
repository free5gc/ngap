...
var _ ProtocolIE = &NRPPaPDU{}

type NRPPaPDU struct {
	Value aper.OctetString
}

func (x *NRPPaPDU) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NRPPaPDU) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *NRPPaPDU) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *NRPPaPDU) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
