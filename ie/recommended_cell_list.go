...
type RecommendedCellList struct {
	List []RecommendedCellItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *RecommendedCellList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *RecommendedCellList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []RecommendedCellItem{}
	for i := 0; i < int(numElements); i++ {
		var val RecommendedCellItem
