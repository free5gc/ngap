...
var _ ProtocolIE = &NGRANTraceID{}

type NGRANTraceID struct {
	Value aper.OctetString // sizeLB:8,sizeUB:8
}

func (x *NGRANTraceID) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NGRANTraceID) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *NGRANTraceID) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *NGRANTraceID) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
