...
var _ ProtocolIE = &MDTPLMNList{}

type MDTPLMNList struct {
	List []PLMNIdentity // sizeLB:1,sizeUB:16
}

func (x *MDTPLMNList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MDTPLMNList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *MDTPLMNList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *MDTPLMNList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
