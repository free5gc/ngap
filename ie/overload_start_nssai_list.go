...
var _ ProtocolIE = &OverloadStartNSSAIList{}

type OverloadStartNSSAIList struct {
	List []OverloadStartNSSAIItem // valueExt,sizeLB:1,sizeUB:1024
}

func (x *OverloadStartNSSAIList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *OverloadStartNSSAIList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 1024
	...
	x.List = []OverloadStartNSSAIItem{}
	for i := 0; i < int(numElements); i++ {
		var val OverloadStartNSSAIItem
...
func (x *OverloadStartNSSAIList) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *OverloadStartNSSAIList) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
