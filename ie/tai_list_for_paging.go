...
var _ ProtocolIE = &TAIListForPaging{}

type TAIListForPaging struct {
	List []TAIListForPagingItem // valueExt,sizeLB:1,sizeUB:16
}

func (x *TAIListForPaging) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *TAIListForPaging) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 16
	...
	x.List = []TAIListForPagingItem{}
	for i := 0; i < int(numElements); i++ {
		var val TAIListForPagingItem
...
func (x *TAIListForPaging) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *TAIListForPaging) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
