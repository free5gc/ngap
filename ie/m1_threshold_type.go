...
type M1ThresholdType struct {
	Choice M1ThresholdTypeAlt
}

type M1ThresholdTypeAlt interface {
	M1ThresholdTypeAltIndex() int64
	Write(*aper.PerBitData) error
	Read(*aper.PerBitData) error
}

// Choice type and its Read/Write is defined elsewhere
func (alt0 ThresholdRSRP) M1ThresholdTypeAltIndex() int64 {
	return int64(0)
}

// Choice type and its Read/Write is defined elsewhere
func (alt1 ThresholdRSRQ) M1ThresholdTypeAltIndex() int64 {
	return int64(1)
}

// Choice type and its Read/Write is defined elsewhere
func (alt2 ThresholdSINR) M1ThresholdTypeAltIndex() int64 {
	return int64(2)
}

// Choice type and its Read/Write is defined elsewhere
func (alt3 ProtocolIESingleContainerM1ThresholdTypeExtIEs) M1ThresholdTypeAltIndex() int64 {
	return int64(3)
}

// Choice Type Read/Write Functions

func (x *M1ThresholdType) Write(pd *aper.PerBitData) error {
	var err error
	...
	var option_idx int64 = x.Choice.M1ThresholdTypeAltIndex()
...
func (x *M1ThresholdType) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	if option_idx == 0 {
		...
		x.Choice = new(ThresholdSINR)
	} else if option_idx == 3 {
		x.Choice = new(ProtocolIESingleContainerM1ThresholdTypeExtIEs)
	} else {
