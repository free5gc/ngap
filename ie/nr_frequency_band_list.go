...
type NRFrequencyBandList struct {
	List []NRFrequencyBandItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *NRFrequencyBandList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NRFrequencyBandList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []NRFrequencyBandItem{}
	for i := 0; i < int(numElements); i++ {
		var val NRFrequencyBandItem
