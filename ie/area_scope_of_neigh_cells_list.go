...
type AreaScopeOfNeighCellsList struct {
	List []AreaScopeOfNeighCellsItem // valueExt,sizeLB:1,sizeUB:8
}

func (x *AreaScopeOfNeighCellsList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *AreaScopeOfNeighCellsList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 8
	...
	x.List = []AreaScopeOfNeighCellsItem{}
	for i := 0; i < int(numElements); i++ {
		var val AreaScopeOfNeighCellsItem
