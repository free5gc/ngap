...
type MBSSessionTNLInfoNGRANList struct {
	List []MBSSessionTNLInfoNGRANItem // valueExt,sizeLB:1,sizeUB:256
}

func (x *MBSSessionTNLInfoNGRANList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSSessionTNLInfoNGRANList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 256
	...
	x.List = []MBSSessionTNLInfoNGRANItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSSessionTNLInfoNGRANItem
