...
type DRBsToQosFlowsMappingList struct {
	List []DRBsToQosFlowsMappingItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *DRBsToQosFlowsMappingList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *DRBsToQosFlowsMappingList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []DRBsToQosFlowsMappingItem{}
	for i := 0; i < int(numElements); i++ {
		var val DRBsToQosFlowsMappingItem
