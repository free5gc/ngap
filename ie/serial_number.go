...
var _ ProtocolIE = &SerialNumber{}

type SerialNumber struct {
	Value aper.BitString // sizeLB:16,sizeUB:16
}

func (x *SerialNumber) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *SerialNumber) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
...
func (x *SerialNumber) WriteIE(pd *aper.PerBitData, id ProtocolIEID, criticality ProtocolIECriticality) error {
...
func (x *SerialNumber) ReadIE(pd *aper.PerBitData) error {
	// Protocol IE (ProtocolIE-Field in 38.413) is a SEQUENCE type
	optPresentFlag := []bool{} // no optional field
