...
type NRCGIListForWarning struct {
	List []NRCGI // valueExt,sizeLB:1,sizeUB:65535
}

func (x *NRCGIListForWarning) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *NRCGIListForWarning) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
