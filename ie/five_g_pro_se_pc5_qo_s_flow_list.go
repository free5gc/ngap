...
type FiveGProSePC5QoSFlowList struct {
	List []FiveGProSePC5QoSFlowItem // valueExt,sizeLB:1,sizeUB:2048
}

func (x *FiveGProSePC5QoSFlowList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *FiveGProSePC5QoSFlowList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 2048
	...
	x.List = []FiveGProSePC5QoSFlowItem{}
	for i := 0; i < int(numElements); i++ {
		var val FiveGProSePC5QoSFlowItem
