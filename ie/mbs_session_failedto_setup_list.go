...
type MBSSessionFailedtoSetupList struct {
	List []MBSSessionFailedtoSetupItem // valueExt,sizeLB:1,sizeUB:32
}

func (x *MBSSessionFailedtoSetupList) Write(pd *aper.PerBitData) error {
	var err error
...
func (x *MBSSessionFailedtoSetupList) Read(pd *aper.PerBitData) error {
	var err error
	var sLb, sUb *uint64 = new(uint64), new(uint64)
	var vLb, vUb *int64 = new(int64), new(int64)
	...
	*sLb, *sUb = 1, 32
	...
	x.List = []MBSSessionFailedtoSetupItem{}
	for i := 0; i < int(numElements); i++ {
		var val MBSSessionFailedtoSetupItem
