...
type fieldParameters struct {
	...
	sizeLowerBound      *uint64 // a sizeLowerBound is the minimum size of type constraint(maybe nil).
	sizeUpperBound      *uint64 // a sizeUpperBound is the maximum size of type constraint(maybe nil).
...
func parseFieldParameters(str string) (params fieldParameters) {
	for _, part := range strings.Split(str, ",") {
		...
		case strings.HasPrefix(part, "sizeLB:"):
			...
			if err == nil {
				params.sizeLowerBound = new(uint64)
				*params.sizeLowerBound = uint64(i)
			}
		case strings.HasPrefix(part, "sizeUB:"):
			i, err := strconv.ParseInt(part[7:], 10, 64)
			if err == nil {
				params.sizeUpperBound = new(uint64)
				*params.sizeUpperBound = uint64(i)
			}
