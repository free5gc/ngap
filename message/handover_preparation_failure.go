...
type HandoverPreparationFailure struct {
	...
	TargettoSourceFailureTransparentContainer *ie.TargettoSourceFailureTransparentContainer // optional
}

func (x *HandoverPreparationFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *HandoverPreparationFailure) ProcedureCode() int64 {
	return ProcedureCodeHandoverPreparation
}

func (x *HandoverPreparationFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *HandoverPreparationFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: HandoverPreparationFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.TargettoSourceFailureTransparentContainer != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.TargettoSourceFailureTransparentContainer != nil {
		err = x.TargettoSourceFailureTransparentContainer.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTargettoSourceFailureTransparentContainer},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *HandoverPreparationFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (HandoverPreparationFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDTargettoSourceFailureTransparentContainer {
			// check if ie is duplicated
			if x.TargettoSourceFailureTransparentContainer != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TargettoSourceFailureTransparentContainer")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TargettoSourceFailureTransparentContainer = &ie.TargettoSourceFailureTransparentContainer{}
			err = x.TargettoSourceFailureTransparentContainer.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
