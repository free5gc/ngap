...
func (x *HandoverSuccess) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *HandoverSuccess) ProcedureCode() int64 {
	return ProcedureCodeHandoverSuccess
}

func (x *HandoverSuccess) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *HandoverSuccess) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: HandoverSuccess (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *HandoverSuccess) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (HandoverSuccess) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
