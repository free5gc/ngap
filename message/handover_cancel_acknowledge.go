...
func (x *HandoverCancelAcknowledge) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *HandoverCancelAcknowledge) ProcedureCode() int64 {
	return ProcedureCodeHandoverCancel
}

func (x *HandoverCancelAcknowledge) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *HandoverCancelAcknowledge) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: HandoverCancelAcknowledge (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *HandoverCancelAcknowledge) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (HandoverCancelAcknowledge) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
