...
type PDUSessionResourceNotify struct {
	...
	PDUSessionResourceNotifyList      *ie.PDUSessionResourceNotifyList      // optional
	PDUSessionResourceReleasedListNot *ie.PDUSessionResourceReleasedListNot // optional
	UserLocationInformation           *ie.UserLocationInformation           // optional
}

func (x *PDUSessionResourceNotify) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PDUSessionResourceNotify) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceNotify
}

func (x *PDUSessionResourceNotify) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *PDUSessionResourceNotify) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceNotify (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceNotifyList != nil {
		numIes++
	}
	if x.PDUSessionResourceReleasedListNot != nil {
		numIes++
	}
	...
	// IE Field 3 (optional)
	if x.PDUSessionResourceNotifyList != nil {
		err = x.PDUSessionResourceNotifyList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceNotifyList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	...
	// IE Field 4 (optional)
	if x.PDUSessionResourceReleasedListNot != nil {
		err = x.PDUSessionResourceReleasedListNot.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceReleasedListNot},
...
func (x *PDUSessionResourceNotify) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceNotify) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceNotifyList {
			// check if ie is duplicated
			if x.PDUSessionResourceNotifyList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceNotifyList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceNotifyList = &ie.PDUSessionResourceNotifyList{}
			err = x.PDUSessionResourceNotifyList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceReleasedListNot {
			// check if ie is duplicated
			if x.PDUSessionResourceReleasedListNot != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceReleasedListNot")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceReleasedListNot = &ie.PDUSessionResourceReleasedListNot{}
			err = x.PDUSessionResourceReleasedListNot.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
