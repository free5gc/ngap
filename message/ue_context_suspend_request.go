...
type UEContextSuspendRequest struct {
	...
	PDUSessionResourceSuspendListSUSReq        *ie.PDUSessionResourceSuspendListSUSReq        // optional
}

func (x *UEContextSuspendRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *UEContextSuspendRequest) ProcedureCode() int64 {
	return ProcedureCodeUEContextSuspend
}

func (x *UEContextSuspendRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UEContextSuspendRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UEContextSuspendRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceSuspendListSUSReq != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.PDUSessionResourceSuspendListSUSReq != nil {
		err = x.PDUSessionResourceSuspendListSUSReq.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceSuspendListSUSReq},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *UEContextSuspendRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UEContextSuspendRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceSuspendListSUSReq {
			// check if ie is duplicated
			if x.PDUSessionResourceSuspendListSUSReq != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceSuspendListSUSReq")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceSuspendListSUSReq = &ie.PDUSessionResourceSuspendListSUSReq{}
			err = x.PDUSessionResourceSuspendListSUSReq.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
