...
type LocationReport struct {
	...
	UEPresenceInAreaOfInterestList *ie.UEPresenceInAreaOfInterestList // optional
	LocationReportingRequestType   *ie.LocationReportingRequestType   // mandatory
}

func (x *LocationReport) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *LocationReport) ProcedureCode() int64 {
	return ProcedureCodeLocationReport
}

func (x *LocationReport) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *LocationReport) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: LocationReport (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.UEPresenceInAreaOfInterestList != nil {
		numIes++
	}
	...
	// IE Field 4 (optional)
	if x.UEPresenceInAreaOfInterestList != nil {
		err = x.UEPresenceInAreaOfInterestList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDUEPresenceInAreaOfInterestList},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	...
	if x.LocationReportingRequestType != nil {
		...
		return pd.Bytes(), errors.Errorf("mandatory field LocationReportingRequestType is missing")
	}
...
func (x *LocationReport) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (LocationReport) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDUEPresenceInAreaOfInterestList {
			// check if ie is duplicated
			if x.UEPresenceInAreaOfInterestList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: UEPresenceInAreaOfInterestList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.UEPresenceInAreaOfInterestList = &ie.UEPresenceInAreaOfInterestList{}
			err = x.UEPresenceInAreaOfInterestList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// LocationReportingRequestType is mandatory but may be nil (ignored)
