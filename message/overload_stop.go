...
func (x *OverloadStop) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *OverloadStop) ProcedureCode() int64 {
	return ProcedureCodeOverloadStop
}

func (x *OverloadStop) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *OverloadStop) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: OverloadStop (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *OverloadStop) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (OverloadStop) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
