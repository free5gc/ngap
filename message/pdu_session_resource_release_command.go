...
type PDUSessionResourceReleaseCommand struct {
	...
	PDUSessionResourceToReleaseListRelCmd *ie.PDUSessionResourceToReleaseListRelCmd // mandatory
}

func (x *PDUSessionResourceReleaseCommand) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PDUSessionResourceReleaseCommand) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceRelease
}

func (x *PDUSessionResourceReleaseCommand) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PDUSessionResourceReleaseCommand) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceReleaseCommand (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceToReleaseListRelCmd != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceToReleaseListRelCmd != nil {
		err = x.PDUSessionResourceToReleaseListRelCmd.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceToReleaseListRelCmd},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceToReleaseListRelCmd is missing")
	}
...
func (x *PDUSessionResourceReleaseCommand) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceReleaseCommand) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceToReleaseListRelCmd {
			// check if ie is duplicated
			if x.PDUSessionResourceToReleaseListRelCmd != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceToReleaseListRelCmd")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceToReleaseListRelCmd = &ie.PDUSessionResourceToReleaseListRelCmd{}
			err = x.PDUSessionResourceToReleaseListRelCmd.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PDUSessionResourceToReleaseListRelCmd == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPDUSessionResourceToReleaseListRelCmd, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PDUSessionResourceToReleaseListRelCmd is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
