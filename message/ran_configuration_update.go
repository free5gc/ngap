...
type RANConfigurationUpdate struct {
	...
	SupportedTAList                 *ie.SupportedTAList                 // optional
	DefaultPagingDRX                *ie.PagingDRX                       // optional
	GlobalRANNodeID                 *ie.GlobalRANNodeID                 // optional
	NGRANTNLAssociationToRemoveList *ie.NGRANTNLAssociationToRemoveList // optional
	NBIoTDefaultPagingDRX           *ie.NBIoTDefaultPagingDRX           // optional
	ExtendedRANNodeName             *ie.ExtendedRANNodeName             // optional
}

func (x *RANConfigurationUpdate) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *RANConfigurationUpdate) ProcedureCode() int64 {
	return ProcedureCodeRANConfigurationUpdate
}

func (x *RANConfigurationUpdate) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *RANConfigurationUpdate) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: RANConfigurationUpdate (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.NGRANTNLAssociationToRemoveList != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.NGRANTNLAssociationToRemoveList != nil {
		err = x.NGRANTNLAssociationToRemoveList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDNGRANTNLAssociationToRemoveList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *RANConfigurationUpdate) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (RANConfigurationUpdate) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDNGRANTNLAssociationToRemoveList {
			// check if ie is duplicated
			if x.NGRANTNLAssociationToRemoveList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: NGRANTNLAssociationToRemoveList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.NGRANTNLAssociationToRemoveList = &ie.NGRANTNLAssociationToRemoveList{}
			err = x.NGRANTNLAssociationToRemoveList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
