...
type MulticastGroupPaging struct {
	...
	MBSServiceArea               *ie.MBSServiceArea               // optional
	MulticastGroupPagingAreaList *ie.MulticastGroupPagingAreaList // mandatory
}

func (x *MulticastGroupPaging) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *MulticastGroupPaging) ProcedureCode() int64 {
	return ProcedureCodeMulticastGroupPaging
}

func (x *MulticastGroupPaging) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *MulticastGroupPaging) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: MulticastGroupPaging (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.MulticastGroupPagingAreaList != nil {
		numIes++
	}
	...
	if x.MulticastGroupPagingAreaList != nil {
		err = x.MulticastGroupPagingAreaList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDMulticastGroupPagingAreaList},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field MulticastGroupPagingAreaList is missing")
	}
...
func (x *MulticastGroupPaging) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MulticastGroupPaging) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDMulticastGroupPagingAreaList {
			// check if ie is duplicated
			if x.MulticastGroupPagingAreaList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: MulticastGroupPagingAreaList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.MulticastGroupPagingAreaList = &ie.MulticastGroupPagingAreaList{}
			err = x.MulticastGroupPagingAreaList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// MBSSessionID is mandatory but may be nil (ignored)

	// MulticastGroupPagingAreaList is mandatory but may be nil (ignored)
