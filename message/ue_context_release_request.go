...
type UEContextReleaseRequest struct {
	...
	PDUSessionResourceListCxtRelReq *ie.PDUSessionResourceListCxtRelReq // optional
	Cause                           *ie.Cause                           // mandatory
}

func (x *UEContextReleaseRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *UEContextReleaseRequest) ProcedureCode() int64 {
	return ProcedureCodeUEContextReleaseRequest
}

func (x *UEContextReleaseRequest) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *UEContextReleaseRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UEContextReleaseRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceListCxtRelReq != nil {
		numIes++
	}
	if x.Cause != nil {
		numIes++
	...
	// IE Field 3 (optional)
	if x.PDUSessionResourceListCxtRelReq != nil {
		err = x.PDUSessionResourceListCxtRelReq.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceListCxtRelReq},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *UEContextReleaseRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UEContextReleaseRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceListCxtRelReq {
			// check if ie is duplicated
			if x.PDUSessionResourceListCxtRelReq != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceListCxtRelReq")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceListCxtRelReq = &ie.PDUSessionResourceListCxtRelReq{}
			err = x.PDUSessionResourceListCxtRelReq.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
