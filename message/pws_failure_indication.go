...
type PWSFailureIndication struct {
	PWSFailedCellIDList *ie.PWSFailedCellIDList // mandatory
	GlobalRANNodeID     *ie.GlobalRANNodeID     // mandatory
}

func (x *PWSFailureIndication) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PWSFailureIndication) ProcedureCode() int64 {
	return ProcedureCodePWSFailureIndication
}

func (x *PWSFailureIndication) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *PWSFailureIndication) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PWSFailureIndication (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PWSFailedCellIDList != nil {
		err = x.PWSFailedCellIDList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPWSFailedCellIDList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PWSFailedCellIDList is missing")
	}
...
func (x *PWSFailureIndication) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PWSFailureIndication) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDPWSFailedCellIDList {
			// check if ie is duplicated
			if x.PWSFailedCellIDList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PWSFailedCellIDList")
			...
			x.PWSFailedCellIDList = &ie.PWSFailedCellIDList{}
			err = x.PWSFailedCellIDList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PWSFailedCellIDList == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPWSFailedCellIDList, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PWSFailedCellIDList is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
