...
type UplinkNASTransport struct {
	...
	WAGFIdentityInformation *aper.OctetString           // optional
	TNGFIdentityInformation *aper.OctetString           // optional
	TWIFIdentityInformation *aper.OctetString           // optional
}

func (x *UplinkNASTransport) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *UplinkNASTransport) ProcedureCode() int64 {
	return ProcedureCodeUplinkNASTransport
}

func (x *UplinkNASTransport) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *UplinkNASTransport) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UplinkNASTransport (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.WAGFIdentityInformation != nil {
		numIes++
	}
	if x.TNGFIdentityInformation != nil {
		numIes++
	}
	if x.TWIFIdentityInformation != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.WAGFIdentityInformation != nil {
		err = ie.WriteOctetStringIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDWAGFIdentityInformation},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject},
			*(x.WAGFIdentityInformation))
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	}
	...
	if x.TNGFIdentityInformation != nil {
		err = ie.WriteOctetStringIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTNGFIdentityInformation},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject},
			*(x.TNGFIdentityInformation))
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	}
	...
	if x.TWIFIdentityInformation != nil {
		err = ie.WriteOctetStringIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTWIFIdentityInformation},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject},
			*(x.TWIFIdentityInformation))
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	}
...
func (x *UplinkNASTransport) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UplinkNASTransport) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDWAGFIdentityInformation {
			// check if ie is duplicated
			if x.WAGFIdentityInformation != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: WAGFIdentityInformation")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.WAGFIdentityInformation = &aper.OctetString{}
			err = ie.ReadOctetStringIE(pdOpenType, x.WAGFIdentityInformation)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 6
		if protocolIeId == ie.ProtocolIEIDTNGFIdentityInformation {
			// check if ie is duplicated
			if x.TNGFIdentityInformation != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TNGFIdentityInformation")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TNGFIdentityInformation = &aper.OctetString{}
			err = ie.ReadOctetStringIE(pdOpenType, x.TNGFIdentityInformation)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 7
		if protocolIeId == ie.ProtocolIEIDTWIFIdentityInformation {
			// check if ie is duplicated
			if x.TWIFIdentityInformation != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TWIFIdentityInformation")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TWIFIdentityInformation = &aper.OctetString{}
			err = ie.ReadOctetStringIE(pdOpenType, x.TWIFIdentityInformation)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
