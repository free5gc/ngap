...
type NGReset struct {
	...
	ResetType *ie.ResetType // mandatory
}

func (x *NGReset) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *NGReset) ProcedureCode() int64 {
	return ProcedureCodeNGReset
}

func (x *NGReset) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *NGReset) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: NGReset (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.ResetType != nil {
		err = x.ResetType.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDResetType},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field ResetType is missing")
	}
...
func (x *NGReset) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (NGReset) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDResetType {
			// check if ie is duplicated
			if x.ResetType != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: ResetType")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.ResetType = &ie.ResetType{}
			err = x.ResetType.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.ResetType == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDResetType, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE ResetType is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
