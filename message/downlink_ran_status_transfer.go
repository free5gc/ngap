...
type DownlinkRANStatusTransfer struct {
	...
	RANStatusTransferTransparentContainer *ie.RANStatusTransferTransparentContainer // mandatory
}

func (x *DownlinkRANStatusTransfer) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *DownlinkRANStatusTransfer) ProcedureCode() int64 {
	return ProcedureCodeDownlinkRANStatusTransfer
}

func (x *DownlinkRANStatusTransfer) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *DownlinkRANStatusTransfer) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: DownlinkRANStatusTransfer (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.RANStatusTransferTransparentContainer != nil {
		numIes++
	}
	...
	if x.RANStatusTransferTransparentContainer != nil {
		err = x.RANStatusTransferTransparentContainer.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDRANStatusTransferTransparentContainer},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field RANStatusTransferTransparentContainer is missing")
	}
...
func (x *DownlinkRANStatusTransfer) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (DownlinkRANStatusTransfer) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDRANStatusTransferTransparentContainer {
			// check if ie is duplicated
			if x.RANStatusTransferTransparentContainer != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: RANStatusTransferTransparentContainer")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.RANStatusTransferTransparentContainer = &ie.RANStatusTransferTransparentContainer{}
			err = x.RANStatusTransferTransparentContainer.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.RANStatusTransferTransparentContainer == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDRANStatusTransferTransparentContainer, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE RANStatusTransferTransparentContainer is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
