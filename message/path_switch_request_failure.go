...
type PathSwitchRequestFailure struct {
	...
	PDUSessionResourceReleasedListPSFail *ie.PDUSessionResourceReleasedListPSFail // mandatory
	CriticalityDiagnostics               *ie.CriticalityDiagnostics               // optional
}

func (x *PathSwitchRequestFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *PathSwitchRequestFailure) ProcedureCode() int64 {
	return ProcedureCodePathSwitchRequest
}

func (x *PathSwitchRequestFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PathSwitchRequestFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PathSwitchRequestFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceReleasedListPSFail != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceReleasedListPSFail != nil {
		err = x.PDUSessionResourceReleasedListPSFail.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceReleasedListPSFail},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceReleasedListPSFail is missing")
	}
...
func (x *PathSwitchRequestFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PathSwitchRequestFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceReleasedListPSFail {
			// check if ie is duplicated
			if x.PDUSessionResourceReleasedListPSFail != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceReleasedListPSFail")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceReleasedListPSFail = &ie.PDUSessionResourceReleasedListPSFail{}
			err = x.PDUSessionResourceReleasedListPSFail.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// PDUSessionResourceReleasedListPSFail is mandatory but may be nil (ignored)
