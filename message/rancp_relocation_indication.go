...
type RANCPRelocationIndication struct {
	...
	EUTRACGI                *ie.EUTRACGI                // mandatory
	TAI                     *ie.TAI                     // mandatory
	ULCPSecurityInformation *ie.ULCPSecurityInformation // mandatory
}

func (x *RANCPRelocationIndication) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *RANCPRelocationIndication) ProcedureCode() int64 {
	return ProcedureCodeRANCPRelocationIndication
}

func (x *RANCPRelocationIndication) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *RANCPRelocationIndication) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: RANCPRelocationIndication (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.ULCPSecurityInformation != nil {
		numIes++
	}
	...
	if x.EUTRACGI != nil {
		err = x.EUTRACGI.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDEUTRACGI},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field EUTRACGI is missing")
	}

	// IE Field 4 (mandatory)
	if x.TAI != nil {
		err = x.TAI.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTAI},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field TAI is missing")
	}

	// IE Field 5 (mandatory)
	if x.ULCPSecurityInformation != nil {
		err = x.ULCPSecurityInformation.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDULCPSecurityInformation},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field ULCPSecurityInformation is missing")
	}
...
func (x *RANCPRelocationIndication) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (RANCPRelocationIndication) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDEUTRACGI {
			// check if ie is duplicated
			if x.EUTRACGI != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: EUTRACGI")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.EUTRACGI = &ie.EUTRACGI{}
			err = x.EUTRACGI.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDTAI {
			// check if ie is duplicated
			if x.TAI != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TAI")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDULCPSecurityInformation {
			// check if ie is duplicated
			if x.ULCPSecurityInformation != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: ULCPSecurityInformation")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.ULCPSecurityInformation = &ie.ULCPSecurityInformation{}
			err = x.ULCPSecurityInformation.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// EUTRACGI is mandatory but may be nil (ignored)

	// TAI is mandatory but may be nil (ignored)

	if x.ULCPSecurityInformation == nil {
		// Missing Mandatory IE (reject)
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDULCPSecurityInformation, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE ULCPSecurityInformation is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
