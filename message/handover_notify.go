...
type HandoverNotify struct {
	...
	NotifySourceNGRANNode   *ie.NotifySourceNGRANNode   // optional
}

func (x *HandoverNotify) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *HandoverNotify) ProcedureCode() int64 {
	return ProcedureCodeHandoverNotification
}

func (x *HandoverNotify) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *HandoverNotify) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: HandoverNotify (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.NotifySourceNGRANNode != nil {
		numIes++
	}
	...
	// IE Field 4 (optional)
	if x.NotifySourceNGRANNode != nil {
		err = x.NotifySourceNGRANNode.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDNotifySourceNGRANNode},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *HandoverNotify) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (HandoverNotify) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDNotifySourceNGRANNode {
			// check if ie is duplicated
			if x.NotifySourceNGRANNode != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: NotifySourceNGRANNode")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.NotifySourceNGRANNode = &ie.NotifySourceNGRANNode{}
			err = x.NotifySourceNGRANNode.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
