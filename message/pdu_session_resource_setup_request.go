...
type PDUSessionResourceSetupRequest struct {
	...
	PDUSessionResourceSetupListSUReq *ie.PDUSessionResourceSetupListSUReq // mandatory
...
func (x *PDUSessionResourceSetupRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PDUSessionResourceSetupRequest) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceSetup
}

func (x *PDUSessionResourceSetupRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PDUSessionResourceSetupRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceSetupRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceSetupListSUReq != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceSetupListSUReq != nil {
		err = x.PDUSessionResourceSetupListSUReq.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceSetupListSUReq},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceSetupListSUReq is missing")
	}
...
func (x *PDUSessionResourceSetupRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceSetupRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceSetupListSUReq {
			// check if ie is duplicated
			if x.PDUSessionResourceSetupListSUReq != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceSetupListSUReq")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceSetupListSUReq = &ie.PDUSessionResourceSetupListSUReq{}
			err = x.PDUSessionResourceSetupListSUReq.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PDUSessionResourceSetupListSUReq == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPDUSessionResourceSetupListSUReq, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PDUSessionResourceSetupListSUReq is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
