...
type SecondaryRATDataUsageReport struct {
	...
	PDUSessionResourceSecondaryRATUsageList *ie.PDUSessionResourceSecondaryRATUsageList // mandatory
	HandoverFlag                            *ie.HandoverFlag                            // optional
	UserLocationInformation                 *ie.UserLocationInformation                 // optional
}

func (x *SecondaryRATDataUsageReport) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *SecondaryRATDataUsageReport) ProcedureCode() int64 {
	return ProcedureCodeSecondaryRATDataUsageReport
}

func (x *SecondaryRATDataUsageReport) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *SecondaryRATDataUsageReport) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: SecondaryRATDataUsageReport (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceSecondaryRATUsageList != nil {
		numIes++
	}
	if x.HandoverFlag != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceSecondaryRATUsageList != nil {
		err = x.PDUSessionResourceSecondaryRATUsageList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceSecondaryRATUsageList},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceSecondaryRATUsageList is missing")
	}

	// IE Field 4 (optional)
	if x.HandoverFlag != nil {
		err = x.HandoverFlag.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDHandoverFlag},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *SecondaryRATDataUsageReport) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (SecondaryRATDataUsageReport) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceSecondaryRATUsageList {
			// check if ie is duplicated
			if x.PDUSessionResourceSecondaryRATUsageList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceSecondaryRATUsageList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceSecondaryRATUsageList = &ie.PDUSessionResourceSecondaryRATUsageList{}
			err = x.PDUSessionResourceSecondaryRATUsageList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDHandoverFlag {
			// check if ie is duplicated
			if x.HandoverFlag != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: HandoverFlag")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.HandoverFlag = &ie.HandoverFlag{}
			err = x.HandoverFlag.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// PDUSessionResourceSecondaryRATUsageList is mandatory but may be nil (ignored)
