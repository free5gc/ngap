...
type UERadioCapabilityCheckResponse struct {
	...
	IMSVoiceSupportIndicator *ie.IMSVoiceSupportIndicator // mandatory
	CriticalityDiagnostics   *ie.CriticalityDiagnostics   // optional
}

func (x *UERadioCapabilityCheckResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *UERadioCapabilityCheckResponse) ProcedureCode() int64 {
	return ProcedureCodeUERadioCapabilityCheck
}

func (x *UERadioCapabilityCheckResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UERadioCapabilityCheckResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UERadioCapabilityCheckResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.IMSVoiceSupportIndicator != nil {
		numIes++
	}
	...
	if x.IMSVoiceSupportIndicator != nil {
		err = x.IMSVoiceSupportIndicator.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDIMSVoiceSupportIndicator},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field IMSVoiceSupportIndicator is missing")
	}
...
func (x *UERadioCapabilityCheckResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UERadioCapabilityCheckResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDIMSVoiceSupportIndicator {
			// check if ie is duplicated
			if x.IMSVoiceSupportIndicator != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: IMSVoiceSupportIndicator")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.IMSVoiceSupportIndicator = &ie.IMSVoiceSupportIndicator{}
			err = x.IMSVoiceSupportIndicator.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.IMSVoiceSupportIndicator == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDIMSVoiceSupportIndicator, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE IMSVoiceSupportIndicator is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
