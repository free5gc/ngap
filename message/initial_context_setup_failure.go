...
type InitialContextSetupFailure struct {
	...
	PDUSessionResourceFailedToSetupListCxtFail *ie.PDUSessionResourceFailedToSetupListCxtFail // optional
	Cause                                      *ie.Cause                                      // mandatory
	CriticalityDiagnostics                     *ie.CriticalityDiagnostics                     // optional
}

func (x *InitialContextSetupFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *InitialContextSetupFailure) ProcedureCode() int64 {
	return ProcedureCodeInitialContextSetup
}

func (x *InitialContextSetupFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *InitialContextSetupFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: InitialContextSetupFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceFailedToSetupListCxtFail != nil {
		numIes++
	}
	if x.Cause != nil {
		numIes++
	...
	// IE Field 3 (optional)
	if x.PDUSessionResourceFailedToSetupListCxtFail != nil {
		err = x.PDUSessionResourceFailedToSetupListCxtFail.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceFailedToSetupListCxtFail},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *InitialContextSetupFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (InitialContextSetupFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceFailedToSetupListCxtFail {
			// check if ie is duplicated
			if x.PDUSessionResourceFailedToSetupListCxtFail != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceFailedToSetupListCxtFail")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceFailedToSetupListCxtFail = &ie.PDUSessionResourceFailedToSetupListCxtFail{}
			err = x.PDUSessionResourceFailedToSetupListCxtFail.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
