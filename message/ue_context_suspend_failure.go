...
func (x *UEContextSuspendFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *UEContextSuspendFailure) ProcedureCode() int64 {
	return ProcedureCodeUEContextSuspend
}

func (x *UEContextSuspendFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UEContextSuspendFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UEContextSuspendFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
...
func (x *UEContextSuspendFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UEContextSuspendFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
