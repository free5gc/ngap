...
type PDUSessionResourceModifyRequest struct {
	...
	PDUSessionResourceModifyListModReq *ie.PDUSessionResourceModifyListModReq // mandatory
}

func (x *PDUSessionResourceModifyRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PDUSessionResourceModifyRequest) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceModify
}

func (x *PDUSessionResourceModifyRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PDUSessionResourceModifyRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceModifyRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceModifyListModReq != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceModifyListModReq != nil {
		err = x.PDUSessionResourceModifyListModReq.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceModifyListModReq},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceModifyListModReq is missing")
	}
...
func (x *PDUSessionResourceModifyRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceModifyRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 4
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceModifyListModReq {
			// check if ie is duplicated
			if x.PDUSessionResourceModifyListModReq != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceModifyListModReq")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceModifyListModReq = &ie.PDUSessionResourceModifyListModReq{}
			err = x.PDUSessionResourceModifyListModReq.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PDUSessionResourceModifyListModReq == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPDUSessionResourceModifyListModReq, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PDUSessionResourceModifyListModReq is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
