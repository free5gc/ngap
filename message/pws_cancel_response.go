...
type PWSCancelResponse struct {
	...
	BroadcastCancelledAreaList *ie.BroadcastCancelledAreaList // optional
	CriticalityDiagnostics     *ie.CriticalityDiagnostics     // optional
}

func (x *PWSCancelResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *PWSCancelResponse) ProcedureCode() int64 {
	return ProcedureCodePWSCancel
}

func (x *PWSCancelResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PWSCancelResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PWSCancelResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.BroadcastCancelledAreaList != nil {
		numIes++
	}
	...
	// IE Field 3 (optional)
	if x.BroadcastCancelledAreaList != nil {
		err = x.BroadcastCancelledAreaList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDBroadcastCancelledAreaList},
...
func (x *PWSCancelResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PWSCancelResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDBroadcastCancelledAreaList {
			// check if ie is duplicated
			if x.BroadcastCancelledAreaList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: BroadcastCancelledAreaList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.BroadcastCancelledAreaList = &ie.BroadcastCancelledAreaList{}
			err = x.BroadcastCancelledAreaList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
