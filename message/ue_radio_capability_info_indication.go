...
type UERadioCapabilityInfoIndication struct {
	...
	UERadioCapabilityEUTRAFormat *ie.UERadioCapability          // optional
}

func (x *UERadioCapabilityInfoIndication) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *UERadioCapabilityInfoIndication) ProcedureCode() int64 {
	return ProcedureCodeUERadioCapabilityInfoIndication
}

func (x *UERadioCapabilityInfoIndication) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *UERadioCapabilityInfoIndication) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UERadioCapabilityInfoIndication (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.UERadioCapabilityEUTRAFormat != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.UERadioCapabilityEUTRAFormat != nil {
		err = x.UERadioCapabilityEUTRAFormat.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDUERadioCapabilityEUTRAFormat},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *UERadioCapabilityInfoIndication) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UERadioCapabilityInfoIndication) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDUERadioCapabilityEUTRAFormat {
			// check if ie is duplicated
			if x.UERadioCapabilityEUTRAFormat != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: UERadioCapabilityEUTRAFormat")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.UERadioCapabilityEUTRAFormat = &ie.UERadioCapability{}
			err = x.UERadioCapabilityEUTRAFormat.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
