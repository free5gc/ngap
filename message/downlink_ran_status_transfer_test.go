...
func TestDownlinkRANStatusTransferMarshalBinary(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		name     string
		input    *DownlinkRANStatusTransfer
		expected []byte
	}{
		{
			...
			expected: []byte{
				0x00, 0x07, 0x40, 0x27, 0x00, 0x00, 0x03, 0x00, 0x0a, 0x00, 0x06, 0x80, 0xce, 0x94, 0x60, 0x9b,
				0x0c, 0x00, 0x55, 0x00, 0x02, 0x00, 0x01, 0x00, 0x54, 0x00, 0x10, 0x00, 0x40, 0x01, 0x7b, 0x00,
...
func TestDownlinkRANStatusTransferUnmarshalBinary(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		name     string
		input    []byte
		expected *DownlinkRANStatusTransfer
	}{
		{
			name: "Case 1: from ueranemu basic-k8s pipeline TestN2Handover",
			input: []byte{
