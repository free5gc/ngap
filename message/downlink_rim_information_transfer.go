...
type DownlinkRIMInformationTransfer struct {
	RIMInformationTransfer *ie.RIMInformationTransfer // optional
}

func (x *DownlinkRIMInformationTransfer) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *DownlinkRIMInformationTransfer) ProcedureCode() int64 {
	return ProcedureCodeDownlinkRIMInformationTransfer
}

func (x *DownlinkRIMInformationTransfer) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *DownlinkRIMInformationTransfer) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: DownlinkRIMInformationTransfer (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	numIes := uint64(0)
	if x.RIMInformationTransfer != nil {
		numIes++
	}
	...
	// IE Field 1 (optional)
	if x.RIMInformationTransfer != nil {
		err = x.RIMInformationTransfer.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDRIMInformationTransfer},
...
func (x *DownlinkRIMInformationTransfer) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (DownlinkRIMInformationTransfer) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDRIMInformationTransfer {
			// check if ie is duplicated
			if x.RIMInformationTransfer != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: RIMInformationTransfer")
			...
			x.RIMInformationTransfer = &ie.RIMInformationTransfer{}
			err = x.RIMInformationTransfer.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
