...
type RANConfigurationUpdateFailure struct {
	...
	TimeToWait             *ie.TimeToWait             // optional
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional
}

func (x *RANConfigurationUpdateFailure) MessageType() int64 {
	return MessageTypeUnsuccessfulOutcome
}

func (x *RANConfigurationUpdateFailure) ProcedureCode() int64 {
	return ProcedureCodeRANConfigurationUpdate
}

func (x *RANConfigurationUpdateFailure) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *RANConfigurationUpdateFailure) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: RANConfigurationUpdateFailure (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 2 (optional)
	if x.TimeToWait != nil {
		err = x.TimeToWait.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDTimeToWait},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *RANConfigurationUpdateFailure) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (RANConfigurationUpdateFailure) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDTimeToWait {
			// check if ie is duplicated
			if x.TimeToWait != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: TimeToWait")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.TimeToWait = &ie.TimeToWait{}
			err = x.TimeToWait.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
