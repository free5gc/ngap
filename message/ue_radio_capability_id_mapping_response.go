...
type UERadioCapabilityIDMappingResponse struct {
	UERadioCapabilityID    *ie.UERadioCapabilityID    // mandatory
	UERadioCapability      *ie.UERadioCapability      // mandatory
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional
}

func (x *UERadioCapabilityIDMappingResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *UERadioCapabilityIDMappingResponse) ProcedureCode() int64 {
	return ProcedureCodeUERadioCapabilityIDMapping
}

func (x *UERadioCapabilityIDMappingResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UERadioCapabilityIDMappingResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UERadioCapabilityIDMappingResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.UERadioCapabilityID != nil {
		...
		return pd.Bytes(), errors.Errorf("mandatory field UERadioCapabilityID is missing")
	}
	...
	if x.UERadioCapability != nil {
		...
		return pd.Bytes(), errors.Errorf("mandatory field UERadioCapability is missing")
	}
...
func (x *UERadioCapabilityIDMappingResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UERadioCapabilityIDMappingResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	if x.UERadioCapabilityID == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDUERadioCapabilityID, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE UERadioCapabilityID is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
	...
	// UERadioCapability is mandatory but may be nil (ignored)
