...
type MulticastSessionUpdateResponse struct {
	...
	MBSAreaSessionID       *ie.MBSAreaSessionID       // optional
	CriticalityDiagnostics *ie.CriticalityDiagnostics // optional
}

func (x *MulticastSessionUpdateResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *MulticastSessionUpdateResponse) ProcedureCode() int64 {
	return ProcedureCodeMulticastSessionUpdate
}

func (x *MulticastSessionUpdateResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *MulticastSessionUpdateResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: MulticastSessionUpdateResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	// IE Field 2 (optional)
	if x.MBSAreaSessionID != nil {
		err = x.MBSAreaSessionID.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDMBSAreaSessionID},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *MulticastSessionUpdateResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (MulticastSessionUpdateResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDMBSAreaSessionID {
			// check if ie is duplicated
			if x.MBSAreaSessionID != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: MBSAreaSessionID")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.MBSAreaSessionID = &ie.MBSAreaSessionID{}
			err = x.MBSAreaSessionID.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
