...
type UEContextReleaseCommand struct {
	UENGAPIDs *ie.UENGAPIDs // mandatory
	Cause     *ie.Cause     // mandatory
}

func (x *UEContextReleaseCommand) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *UEContextReleaseCommand) ProcedureCode() int64 {
	return ProcedureCodeUEContextRelease
}

func (x *UEContextReleaseCommand) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UEContextReleaseCommand) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UEContextReleaseCommand (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.UENGAPIDs != nil {
		err = x.UENGAPIDs.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDUENGAPIDs},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field UENGAPIDs is missing")
	}
...
func (x *UEContextReleaseCommand) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UEContextReleaseCommand) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDUENGAPIDs {
			// check if ie is duplicated
			if x.UENGAPIDs != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: UENGAPIDs")
			...
			x.UENGAPIDs = &ie.UENGAPIDs{}
			err = x.UENGAPIDs.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.UENGAPIDs == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDUENGAPIDs, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE UENGAPIDs is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
