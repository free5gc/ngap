...
type DownlinkRANConfigurationTransfer struct {
	SONConfigurationTransferDL            *ie.SONConfigurationTransfer            // optional
	ENDCSONConfigurationTransferDL        *ie.ENDCSONConfigurationTransfer        // optional
	IntersystemSONConfigurationTransferDL *ie.IntersystemSONConfigurationTransfer // optional
}

func (x *DownlinkRANConfigurationTransfer) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *DownlinkRANConfigurationTransfer) ProcedureCode() int64 {
	return ProcedureCodeDownlinkRANConfigurationTransfer
}

func (x *DownlinkRANConfigurationTransfer) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *DownlinkRANConfigurationTransfer) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: DownlinkRANConfigurationTransfer (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	numIes := uint64(0)
	if x.SONConfigurationTransferDL != nil {
		numIes++
	}
	if x.ENDCSONConfigurationTransferDL != nil {
		numIes++
	}
	if x.IntersystemSONConfigurationTransferDL != nil {
		numIes++
	}
	...
	// IE Field 1 (optional)
	if x.SONConfigurationTransferDL != nil {
		err = x.SONConfigurationTransferDL.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDSONConfigurationTransferDL},
	...
	// IE Field 2 (optional)
	if x.ENDCSONConfigurationTransferDL != nil {
		err = x.ENDCSONConfigurationTransferDL.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDENDCSONConfigurationTransferDL},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	...
	// IE Field 3 (optional)
	if x.IntersystemSONConfigurationTransferDL != nil {
		err = x.IntersystemSONConfigurationTransferDL.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDIntersystemSONConfigurationTransferDL},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *DownlinkRANConfigurationTransfer) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (DownlinkRANConfigurationTransfer) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDSONConfigurationTransferDL {
			// check if ie is duplicated
			if x.SONConfigurationTransferDL != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: SONConfigurationTransferDL")
			...
			x.SONConfigurationTransferDL = &ie.SONConfigurationTransfer{}
			err = x.SONConfigurationTransferDL.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDENDCSONConfigurationTransferDL {
			// check if ie is duplicated
			if x.ENDCSONConfigurationTransferDL != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: ENDCSONConfigurationTransferDL")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.ENDCSONConfigurationTransferDL = &ie.ENDCSONConfigurationTransfer{}
			err = x.ENDCSONConfigurationTransferDL.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDIntersystemSONConfigurationTransferDL {
			// check if ie is duplicated
			if x.IntersystemSONConfigurationTransferDL != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: IntersystemSONConfigurationTransferDL")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.IntersystemSONConfigurationTransferDL = &ie.IntersystemSONConfigurationTransfer{}
			err = x.IntersystemSONConfigurationTransferDL.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
