...
type DownlinkRANEarlyStatusTransfer struct {
	...
	EarlyStatusTransferTransparentContainer *ie.EarlyStatusTransferTransparentContainer // mandatory
}

func (x *DownlinkRANEarlyStatusTransfer) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *DownlinkRANEarlyStatusTransfer) ProcedureCode() int64 {
	return ProcedureCodeDownlinkRANEarlyStatusTransfer
}

func (x *DownlinkRANEarlyStatusTransfer) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *DownlinkRANEarlyStatusTransfer) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: DownlinkRANEarlyStatusTransfer (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.EarlyStatusTransferTransparentContainer != nil {
		numIes++
	}
	...
	if x.EarlyStatusTransferTransparentContainer != nil {
		err = x.EarlyStatusTransferTransparentContainer.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDEarlyStatusTransferTransparentContainer},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field EarlyStatusTransferTransparentContainer is missing")
	}
...
func (x *DownlinkRANEarlyStatusTransfer) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (DownlinkRANEarlyStatusTransfer) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDEarlyStatusTransferTransparentContainer {
			// check if ie is duplicated
			if x.EarlyStatusTransferTransparentContainer != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: EarlyStatusTransferTransparentContainer")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.EarlyStatusTransferTransparentContainer = &ie.EarlyStatusTransferTransparentContainer{}
			err = x.EarlyStatusTransferTransparentContainer.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.EarlyStatusTransferTransparentContainer == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDEarlyStatusTransferTransparentContainer, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE EarlyStatusTransferTransparentContainer is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
