...
type TraceStart struct {
	...
	TraceActivation *ie.TraceActivation // mandatory
}

func (x *TraceStart) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *TraceStart) ProcedureCode() int64 {
	return ProcedureCodeTraceStart
}

func (x *TraceStart) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *TraceStart) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: TraceStart (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.TraceActivation != nil {
		...
		return pd.Bytes(), errors.Errorf("mandatory field TraceActivation is missing")
	}
...
func (x *TraceStart) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (TraceStart) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	// TraceActivation is mandatory but may be nil (ignored)
