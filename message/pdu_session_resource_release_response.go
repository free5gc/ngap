...
type PDUSessionResourceReleaseResponse struct {
	...
	PDUSessionResourceReleasedListRelRes *ie.PDUSessionResourceReleasedListRelRes // mandatory
	UserLocationInformation              *ie.UserLocationInformation              // optional
	CriticalityDiagnostics               *ie.CriticalityDiagnostics               // optional
}

func (x *PDUSessionResourceReleaseResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *PDUSessionResourceReleaseResponse) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceRelease
}

func (x *PDUSessionResourceReleaseResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PDUSessionResourceReleaseResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceReleaseResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceReleasedListRelRes != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceReleasedListRelRes != nil {
		err = x.PDUSessionResourceReleasedListRelRes.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceReleasedListRelRes},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceReleasedListRelRes is missing")
	}
...
func (x *PDUSessionResourceReleaseResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceReleaseResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceReleasedListRelRes {
			// check if ie is duplicated
			if x.PDUSessionResourceReleasedListRelRes != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceReleasedListRelRes")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceReleasedListRelRes = &ie.PDUSessionResourceReleasedListRelRes{}
			err = x.PDUSessionResourceReleasedListRelRes.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	// PDUSessionResourceReleasedListRelRes is mandatory but may be nil (ignored)
