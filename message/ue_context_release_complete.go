...
type UEContextReleaseComplete struct {
	...
	PDUSessionResourceListCxtRelCpl            *ie.PDUSessionResourceListCxtRelCpl            // optional
...
func (x *UEContextReleaseComplete) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *UEContextReleaseComplete) ProcedureCode() int64 {
	return ProcedureCodeUEContextRelease
}

func (x *UEContextReleaseComplete) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *UEContextReleaseComplete) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: UEContextReleaseComplete (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceListCxtRelCpl != nil {
		numIes++
	}
	...
	// IE Field 5 (optional)
	if x.PDUSessionResourceListCxtRelCpl != nil {
		err = x.PDUSessionResourceListCxtRelCpl.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceListCxtRelCpl},
...
func (x *UEContextReleaseComplete) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (UEContextReleaseComplete) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 5
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceListCxtRelCpl {
			// check if ie is duplicated
			if x.PDUSessionResourceListCxtRelCpl != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceListCxtRelCpl")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceListCxtRelCpl = &ie.PDUSessionResourceListCxtRelCpl{}
			err = x.PDUSessionResourceListCxtRelCpl.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
