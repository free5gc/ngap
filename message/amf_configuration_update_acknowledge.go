...
type AMFConfigurationUpdateAcknowledge struct {
	AMFTNLAssociationSetupList         *ie.AMFTNLAssociationSetupList // optional
	AMFTNLAssociationFailedToSetupList *ie.TNLAssociationList         // optional
	CriticalityDiagnostics             *ie.CriticalityDiagnostics     // optional
}

func (x *AMFConfigurationUpdateAcknowledge) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *AMFConfigurationUpdateAcknowledge) ProcedureCode() int64 {
	return ProcedureCodeAMFConfigurationUpdate
}

func (x *AMFConfigurationUpdateAcknowledge) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *AMFConfigurationUpdateAcknowledge) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: AMFConfigurationUpdateAcknowledge (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	numIes := uint64(0)
	if x.AMFTNLAssociationSetupList != nil {
		numIes++
	}
	if x.AMFTNLAssociationFailedToSetupList != nil {
		numIes++
	}
	...
	// IE Field 1 (optional)
	if x.AMFTNLAssociationSetupList != nil {
		err = x.AMFTNLAssociationSetupList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDAMFTNLAssociationSetupList},
	...
	// IE Field 2 (optional)
	if x.AMFTNLAssociationFailedToSetupList != nil {
		err = x.AMFTNLAssociationFailedToSetupList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDAMFTNLAssociationFailedToSetupList},
			ie.ProtocolIECriticality{Value: ie.CriticalityIgnore})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
...
func (x *AMFConfigurationUpdateAcknowledge) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (AMFConfigurationUpdateAcknowledge) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDAMFTNLAssociationSetupList {
			// check if ie is duplicated
			if x.AMFTNLAssociationSetupList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: AMFTNLAssociationSetupList")
			...
			x.AMFTNLAssociationSetupList = &ie.AMFTNLAssociationSetupList{}
			err = x.AMFTNLAssociationSetupList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 2
		if protocolIeId == ie.ProtocolIEIDAMFTNLAssociationFailedToSetupList {
			// check if ie is duplicated
			if x.AMFTNLAssociationFailedToSetupList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: AMFTNLAssociationFailedToSetupList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.AMFTNLAssociationFailedToSetupList = &ie.TNLAssociationList{}
			err = x.AMFTNLAssociationFailedToSetupList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
