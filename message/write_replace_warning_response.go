...
type WriteReplaceWarningResponse struct {
	...
	BroadcastCompletedAreaList *ie.BroadcastCompletedAreaList // optional
	CriticalityDiagnostics     *ie.CriticalityDiagnostics     // optional
}

func (x *WriteReplaceWarningResponse) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *WriteReplaceWarningResponse) ProcedureCode() int64 {
	return ProcedureCodeWriteReplaceWarning
}

func (x *WriteReplaceWarningResponse) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *WriteReplaceWarningResponse) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: WriteReplaceWarningResponse (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.BroadcastCompletedAreaList != nil {
		numIes++
	}
	...
	// IE Field 3 (optional)
	if x.BroadcastCompletedAreaList != nil {
		err = x.BroadcastCompletedAreaList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDBroadcastCompletedAreaList},
...
func (x *WriteReplaceWarningResponse) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (WriteReplaceWarningResponse) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDBroadcastCompletedAreaList {
			// check if ie is duplicated
			if x.BroadcastCompletedAreaList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: BroadcastCompletedAreaList")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.BroadcastCompletedAreaList = &ie.BroadcastCompletedAreaList{}
			err = x.BroadcastCompletedAreaList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
