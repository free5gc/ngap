...
type AMFStatusIndication struct {
	UnavailableGUAMIList *ie.UnavailableGUAMIList // mandatory
}

func (x *AMFStatusIndication) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *AMFStatusIndication) ProcedureCode() int64 {
	return ProcedureCodeAMFStatusIndication
}

func (x *AMFStatusIndication) Criticality() aper.Enumerated {
	return CriticalityIgnore
}

func (x *AMFStatusIndication) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: AMFStatusIndication (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.UnavailableGUAMIList != nil {
		err = x.UnavailableGUAMIList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDUnavailableGUAMIList},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field UnavailableGUAMIList is missing")
	}
...
func (x *AMFStatusIndication) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (AMFStatusIndication) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDUnavailableGUAMIList {
			// check if ie is duplicated
			if x.UnavailableGUAMIList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: UnavailableGUAMIList")
			...
			x.UnavailableGUAMIList = &ie.UnavailableGUAMIList{}
			err = x.UnavailableGUAMIList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.UnavailableGUAMIList == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDUnavailableGUAMIList, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE UnavailableGUAMIList is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
