...
type BroadcastSessionSetupRequest struct {
	MBSSessionID   *ie.MBSSessionID   // mandatory
	SNSSAI         *ie.SNSSAI         // mandatory
	MBSServiceArea *ie.MBSServiceArea // mandatory
}

func (x *BroadcastSessionSetupRequest) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *BroadcastSessionSetupRequest) ProcedureCode() int64 {
	return ProcedureCodeBroadcastSessionSetup
}

func (x *BroadcastSessionSetupRequest) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *BroadcastSessionSetupRequest) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: BroadcastSessionSetupRequest (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.MBSSessionID != nil {
		err = x.MBSSessionID.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDMBSSessionID},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field MBSSessionID is missing")
	}
	...
	if x.SNSSAI != nil {
		...
		return pd.Bytes(), errors.Errorf("mandatory field SNSSAI is missing")
	}

	// IE Field 3 (mandatory)
	if x.MBSServiceArea != nil {
		err = x.MBSServiceArea.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDMBSServiceArea},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field MBSServiceArea is missing")
	}
...
func (x *BroadcastSessionSetupRequest) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (BroadcastSessionSetupRequest) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDMBSSessionID {
			// check if ie is duplicated
			if x.MBSSessionID != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: MBSSessionID")
			...
			x.MBSSessionID = &ie.MBSSessionID{}
			err = x.MBSSessionID.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDMBSServiceArea {
			// check if ie is duplicated
			if x.MBSServiceArea != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: MBSServiceArea")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.MBSServiceArea = &ie.MBSServiceArea{}
			err = x.MBSServiceArea.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.MBSSessionID == nil {
		...
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDMBSSessionID, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE MBSSessionID is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
	...
	if x.SNSSAI == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDSNSSAI, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE SNSSAI is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
	...
	if x.MBSServiceArea == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDMBSServiceArea, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE MBSServiceArea is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
