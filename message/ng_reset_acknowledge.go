...
type NGResetAcknowledge struct {
	UEAssociatedLogicalNGConnectionList *ie.UEAssociatedLogicalNGConnectionList // optional
	CriticalityDiagnostics              *ie.CriticalityDiagnostics              // optional
}

func (x *NGResetAcknowledge) MessageType() int64 {
	return MessageTypeSuccessfulOutcome
}

func (x *NGResetAcknowledge) ProcedureCode() int64 {
	return ProcedureCodeNGReset
}

func (x *NGResetAcknowledge) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *NGResetAcknowledge) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: NGResetAcknowledge (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	numIes := uint64(0)
	if x.UEAssociatedLogicalNGConnectionList != nil {
		numIes++
	}
	...
	// IE Field 1 (optional)
	if x.UEAssociatedLogicalNGConnectionList != nil {
		err = x.UEAssociatedLogicalNGConnectionList.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDUEAssociatedLogicalNGConnectionList},
...
func (x *NGResetAcknowledge) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (NGResetAcknowledge) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 1
		if protocolIeId == ie.ProtocolIEIDUEAssociatedLogicalNGConnectionList {
			// check if ie is duplicated
			if x.UEAssociatedLogicalNGConnectionList != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: UEAssociatedLogicalNGConnectionList")
			...
			x.UEAssociatedLogicalNGConnectionList = &ie.UEAssociatedLogicalNGConnectionList{}
			err = x.UEAssociatedLogicalNGConnectionList.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
