...
type PDUSessionResourceModifyIndication struct {
	...
	PDUSessionResourceModifyListModInd *ie.PDUSessionResourceModifyListModInd // mandatory
	UserLocationInformation            *ie.UserLocationInformation            // optional
}

func (x *PDUSessionResourceModifyIndication) MessageType() int64 {
	return MessageTypeInitiatingMessage
}

func (x *PDUSessionResourceModifyIndication) ProcedureCode() int64 {
	return ProcedureCodePDUSessionResourceModifyIndication
}

func (x *PDUSessionResourceModifyIndication) Criticality() aper.Enumerated {
	return CriticalityReject
}

func (x *PDUSessionResourceModifyIndication) MarshalBinary() ([]byte, error) {
	...
	*vLb, *vUb = 0, 2
	...
	// open type value: PDUSessionResourceModifyIndication (SEQUENCE)
	optPresentFlag = []bool{} // no optional field
	...
	*sLb, *sUb = 0, 65535
	...
	if x.PDUSessionResourceModifyListModInd != nil {
		numIes++
	}
	...
	if x.PDUSessionResourceModifyListModInd != nil {
		err = x.PDUSessionResourceModifyListModInd.WriteIE(pdOpenType,
			ie.ProtocolIEID{Value: ie.ProtocolIEIDPDUSessionResourceModifyListModInd},
			ie.ProtocolIECriticality{Value: ie.CriticalityReject})
		if err != nil {
			return pd.Bytes(), errors.Wrap(err, "message marshal failed")
		}
	} else {
		return pd.Bytes(), errors.Errorf("mandatory field PDUSessionResourceModifyListModInd is missing")
	}
...
func (x *PDUSessionResourceModifyIndication) UnmarshalBinary(marshalled []byte) error {
	...
	if err != nil {
		return ie.BuildTransferSyntaxErr(errors.Wrap(err,
			"asn.1 decode message Value (PDUSessionResourceModifyIndication) sequence error"))
	}
	// sequence element: ProtocolIE-Container (SEQUENCE OF)
	*sLb, *sUb = 0, 65535
	// get number of ies
	...
	for i := 0; i < int(numIes); i++ {
		...
		// IE Field 3
		if protocolIeId == ie.ProtocolIEIDPDUSessionResourceModifyListModInd {
			// check if ie is duplicated
			if x.PDUSessionResourceModifyListModInd != nil {
				// Build Abstract Syntax Error (Falsely Constructed Message)
				errTrace := errors.Errorf("Duplicated IE: PDUSessionResourceModifyListModInd")
				return ie.BuildAbstractSyntaxErr(
					x.ProcedureCode(),
					aper.Enumerated(x.MessageType()),
					x.Criticality(),
			...
			x.PDUSessionResourceModifyListModInd = &ie.PDUSessionResourceModifyListModInd{}
			err = x.PDUSessionResourceModifyListModInd.ReadIE(pdOpenType)
			if err != nil {
				return errors.Wrap(err, "message unmarshal failed")
			}
			continue
	...
	if x.PDUSessionResourceModifyListModInd == nil {
		...
		// Build Abstract Syntax Error (missing IE/IE Group)
		// TODO: support reporting more than one abstract syntax error
		reportIe := ie.BuildAbstractSyntaxErrReportIe(ie.ProtocolIEIDPDUSessionResourceModifyListModInd, ie.CriticalityReject)
		errTrace := errors.Errorf("mandatory IE PDUSessionResourceModifyListModInd is missing")
		return ie.BuildAbstractSyntaxErr(
			x.ProcedureCode(),
			aper.Enumerated(x.MessageType()),
			x.Criticality(),
